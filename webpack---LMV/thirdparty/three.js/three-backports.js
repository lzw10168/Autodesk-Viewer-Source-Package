import {
    ...
    definePropertySafe
} from './three-backports/backport-utils';

import {
    defineBoxBufferGeometry
} from './three-backports/BoxBufferGeometry';
import {
    defineCircleBufferGeometry
} from './three-backports/CircleBufferGeometry';
import {
    defineCylinderBufferGeometry
} from './three-backports/CylinderBufferGeometry';
import {
    definePolyhedronBufferGeometry
} from './three-backports/PolyhedronBufferGeometry';
import {
    defineOctahedronBufferGeometry
} from './three-backports/OctahedronBufferGeometry';
import {
    defineSphereBufferGeometry
} from './three-backports/SphereBufferGeometry';
import {
    defineTorusBufferGeometry
} from './three-backports/TorusBufferGeometry';
...
export const polyfillTHREE = (THREE) => {
    ...
    defineFunctionIfMissing(THREE ? .Box2 ? .prototype, "intersectsBox", function(target) {
        return this.isIntersectionBox(target);
    });
    defineFunctionIfMissing(THREE ? .Box3 ? .prototype, "intersectsBox", function(target) {
        return this.isIntersectionBox(target);
    });
    defineFunctionIfMissing(THREE ? .Ray ? .prototype, "intersectsBox", function(target) {
        return this.isIntersectionBox(target);
    });
    ...
    definePropertySafe(THREE ? .BufferAttribute ? .prototype, 'itemOffset', {
        get: function get() {
            return this.offset;
        },
    ...
    if (typeof THREE.Line === "function") {
        defineFunctionIfMissing(THREE, "LineSegments",
    ...
    definePropertySafe(THREE ? .MeshPhongMaterial ? .prototype, 'flatShading', {
        get: function get() {
            return this.shading === THREE.FlatShading;
        },
        set: function set(val) {
            this.shading = (val === true ? THREE.FlatShading : THREE.SmoothShading);
        }
    });
    ...
    // Polyfill for r137 Quaternion.invert: https://github.com/mrdoob/three.js/blob/3daf128f34da9cdb1e339766a4aae21b5989f131/src/math/Quaternion.js#L607
    defineFunctionIfMissing(THREE ? .Quaternion ? .prototype, "slerpQuaternions", function inslerpQuaternionsvert(qa, qb, t) {
