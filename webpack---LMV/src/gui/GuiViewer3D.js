...
import {
    Prefs
} from '../application/PreferenceNames';
import {
    Viewer3D
} from "../application/Viewer3D";
import {
    getGlobal,
    isFullscreenEnabled,
    isMobileDevice,
...
import {
    setLanguage
} from "../globalization/i18init";
import i18n from "i18next";
import {
    Lang
} from "../globalization/langs";
import {
    getParameterByName
} from "../globals";
import {
...
import {
    RenderOptionsPanel
} from "./RenderOptionsPanel";
import {
    SettingsPanel
} from "./SettingsPanel";
import {
    ViewerToolBar
} from "./toolbar/ViewerToolBar";
import {
    ViewerSettingsPanel
} from "./ViewerSettingsPanel";
import {
    ProgressState
...
export function GuiViewer3D(container, config) {
    ...
    this.progressInModelLoad = false;
...
GuiViewer3D.prototype.loadModel = function(url, options, onSuccessCallback, onErrorCallback, initAfterWorker) {
    ...
    /**
     * 
     * @param vtr - object parsed from ViewpointTreeRoot received from extractor
     * @param model
     */
    function addViewpointTreeRootBubble(vtr, model) {

        let availableViews = model.getDocumentNode() ? .getNamedViews();

        // Remove creating bubbleNode for viewpoints that are already added through named views.
        // Just update their isViewpoint property to true.
        function traverseBubble(node, parentNode, index) {
            if (node.children) {
                for (let i = node.children.length - 1; i >= 0; i--) {
                    traverseBubble(node.children[i], node, i);
                }
            } else {
                let view = availableViews.find(view => view.data.name == node.name);
                if (view) {
                    view.data.isViewpoint = true;
                    view.data.entry = node.entry;
        ...
        if (availableViews) {
            // Iterate from the end of the array as we will be deleting elements as we traverse. 
            // (make sure we don't miss reading an element due to index re-write)
            let len = vtr.children ? .length;

            while (len--) {
                traverseBubble(vtr.children[len], vtr, len); // overview, vtr, len
            }
        }

        // Add remaining viewpoints to document browser
        let documentRoot = model.getDocumentNode() ? .getRootNode();
        if (documentRoot) {
            const vtrBubble = new Autodesk.Viewing.BubbleNode(vtr);
            vtrBubble.parent = documentRoot.children[0];
            documentRoot.children[0].children.push(vtrBubble);
        }
    }
    ...
    options = { ...options,
        'addViews': addViewpointTreeRootBubble
    };
...
GuiViewer3D.prototype.createUI = function(model, force) {
    ...
    this.checkGeometry = function(event) {
        ...
        setTimeout(function() {
            ...
            // Viewer may be disposed or the view may have been reconfigured to 2d meanwhile.
            if (!viewer.impl || viewer.impl.is2d) {
                return;
            }
    ...
    if (model.is2d()) {
        ...
        setTimeout(function() {
            if (!viewer.impl) return; // Check if viewer was unloaded in the meantime
        ...
        if (model.isLoadDone()) {
            ...
            this.addEventListener(et.GEOMETRY_LOADED_EVENT, this.checkGeometry, {
                once: true
            });
        }
    }
...
GuiViewer3D.prototype.onProgressBarUpdate = function(event) {
    if (event.state == ProgressState.ROOT_LOADED || event.state == ProgressState.LOADING) {
        // consider the time between any ROOT_LOADED/LOADING event and 100% as the model load time
        // Notes: - if the 100% event is not received, other progress will never be shown.
        //          This may happen if load is aborted. Currently, there is no notification for an aborted load.
        //          Also, it seems that an abort only happens when the viewer is closed.
        //        - if there are multiple models loading concurrently, might show other progresses after one
        //          model finished and before some other model sends another load event. Multiple models 
        //          have never been well supported as the progress bar would jump between individual model 
        //          progresses.
        this.progressInModelLoad = event.percent < 100;
    } else if (this.progressInModelLoad) {
        // if any loading is happening, ignore other progress bar updates
        return;
    }
    if (event.percent >= 0) {
...
GuiViewer3D.prototype.createSettingsPanel = function(model) {
    ...
    this.dispatchEvent({
        type: SETTINGS_PANEL_CREATED_EVENT
    });
};
...
GuiViewer3D.prototype.initModelStats = function() {
    ...
    function updateModelStatContent(message) {
        ...
        if (model) {
            ...
            text += "Geoms:&nbsp;" + viewer.modelQueue().getGeometryList().getCount() + "<br>";
