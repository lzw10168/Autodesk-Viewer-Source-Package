...
VisibilityManager.prototype.isolateMultiple = function(nodeList) {
    ...
    if (!nodeList || nodeList.length == 0) {
        ...
        for (var i = 0; i < nodeList.length; i++) {
            this.setVisibilityOnNode(nodeList[i], true, i !== lastIndex);
        }
    }
...
VisibilityManager.prototype.hide = function(node, fireEvent = true) {
    ...
    if (Array.isArray(node)) {
        ...
        for (var i = 0; i < node.length; ++i) {
            this.setVisibilityOnNode(node[i], false, i !== lastIndex);
        }
...
VisibilityManager.prototype.show = function(node) {
    ...
    if (Array.isArray(node)) {
        const lastIndex = node.length - 1;
        for (var i = 0; i < node.length; ++i) {
            this.setVisibilityOnNode(node[i], true, i !== lastIndex);
        }
...
VisibilityManager.prototype.toggleVisibility = function(node) {
    ...
    var event = {
        type: hidden ? et.SHOW_EVENT : et.HIDE_EVENT,
        nodeIdArray: [node],
        model: this.model
    };
    this.viewerImpl.api.dispatchEvent(event);
...
VisibilityManager.prototype.setVisibilityOnNode = function(node, visible, skipEventing = false, setRecursive = true) {
    ...
    viewer.sceneUpdated(true, false, skipEventing);
...
MultiModelVisibilityManager.prototype.setVisibilityOnNode = function(node, visible, model, skipEventing = false, setRecursive = true) {
    if (!model) {
        this.warn();
        model = this.models[0];
    }
