...
export function Navigation(camera) {
    ...
    /**
     * Sets the current view to the given LookAt, while keeping other camera properties as is.
     * Jumps to the new location, without camera path animation.
     * @param {THREE.Vector3} from - The desired camera position in world space.
     * @param {THREE.Vector3} to - The desired camera target in world space.
     * @param {THREE.Vector3=} up - The desired camera up direction in world space.
     */
    this.setView = function(from, to, up = undefined) {
        if (!_camera) {
            return;
        }
        ...
        _camera.position.copy(from);
        _camera.target.copy(to);
        if (up != undefined) {
            _camera.up.copy(up);
        }
    ...
    /**
     * @returns {number} the current limit when zooming out 2D vectorized models.
     */
...
Navigation.prototype.setTransitionActive = function(state) {
    ...
    if (changed) {
        this.fireEvent({
            type: Events.TRANSITION_ACTIVE_FLAG_CHANGED,
            transitionActive: !!state
        });
    }
};
