...
export function ToolController(viewerImpl, viewerApi, autocam, utilities, defaultHandler) {
        ...
        for (var i = _toolStack.length; --i >= 0;) {
            if (_toolStack[i].activeName === toolName) {
                ...
                if (!tool.count || tool.count <= 1) {
                    if (tool.count != 1) {
                        console.warn("Deactivating tool without activation: " + toolName);
                    }
                    _toolStack.splice(i, 1);
                }
    ...
    this.applyKeyMappings = function(event) {
        switch (event.keyCode) {
            case KeyCode.EQUALSMOZ:
                return KeyCode.EQUALS;

            case KeyCode.DASHMOZNEW:
            case KeyCode.DASHMOZ:
                return KeyCode.DASH;
        }
...
Object.defineProperty(ToolController, '_lastTouchedElement', {
    value: null,
    writable: true
});
