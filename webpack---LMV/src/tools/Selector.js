...
export function Selector(viewer, model) {
    ...
    this.update2DSelectionHighlighting = function() {
        ...
        // Make sure the 2d selection overlay is initialized
        if (!viewer.overlayScenes[viewer.selection2dOverlayName(model)])
            return;

        // We will mark all selected objects again. So, we first have to unmark the nodes which are already highlighted.
...
export function MultiModelSelector(viewer) {
    ...
    function fireAggregateSelectionChangedEvent(mouseButton) {
    ...
    this.setSelection = function(dbNodeArray, model, selectionType, mouseButton) {
        if (this.selectionDisabled) {
            return;
        }
        ...
        fireAggregateSelectionChangedEvent(mouseButton);
    };


    this.setAggregateSelection = function(selection, fireEvent = true) {
