...
export class UnifiedCamera extends THREE.Camera {
    ...
    setView(viewParams) {
        ...
        if (viewParams.orthoScale !== undefined) {
            this.orthoScale = viewParams.orthoScale;
            const dir = this.target.clone().sub(this.position);
            // Fix target distance so it matches orthoScale
            if (!this.isPerspective && dir.length() !== this.orthoScale) {
                dir.normalize().multiplyScalar(this.orthoScale);
                this.target.copy(dir.add(this.position));
            }
        }
...
UnifiedCamera.adjustOrthoCamera = function(camera, sceneBox) {
    ...
    if (!camera.isPerspective) {
        ...
        // getSize returns 0 for empty boxes, and flat boxes (e.g. 1x1x0) are considered empty
        if (size > 0 && targetDistance > 1000 * size.length()) {
