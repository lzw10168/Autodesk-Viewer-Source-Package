...
export function DefaultHandler(viewerImpl, navapi, utilities) {
    ...
    this.handleAction = function(actionArray, rayData, mouseButton) {
        for (var i = 0; i < actionArray.length; ++i) {
            switch (actionArray[i]) {
                case "selectOnly":
                    if (viewerImpl.selector) {
                        if (rayData) {
                            viewerImpl.selector.setSelection([rayData.dbId], rayData.model, undefined, mouseButton);
                        }
                    }
                    break;
                case "deselectAll":
    ...
    this.handleSingleClick = function(event, button) {
        ...
        const isRightBtnEnabled = viewerImpl.isRightBtnSelectionEnabled();

        var control = event.ctrlKey || event.metaKey;
        var shift = event.shiftKey;
        var alt = event.altKey;

        // Select with either left button or right button with isRightBtnSelectionEnabled
        if (button === 0 || (button === 2 && isRightBtnEnabled)) {
            ...
            if (this.clickConfig && this.clickConfig[key] && this.clickConfig[key][objectKey]) {
                this.handleAction(this.clickConfig[key][objectKey], result, button);
                return true;
            }
