...
import {
    RenderScene
} from "../wgs/scene/RenderScene";
import {
    RenderContext
} from "../wgs/render/RenderContext";
import {
    MaterialManager
} from "../wgs/render/MaterialManager";
import {
    MultiModelSelector
} from "../tools/Selector";
import {
    MultiModelVisibilityManager
} from "../tools/VisibilityManager";
import {
    LightPresets,
    DefaultLightPreset,
...
import {
    GroundShadow
} from "../wgs/render/GroundShadow";
import {
    OtgResourceCache
} from "../file-loaders/main/OtgResourceCache";
import {
    logger
} from "../logger/Logger";
import {
    TextureLoader
} from "../file-loaders/main/TextureLoader";
import {
    unconsolidateModels,
    reconsolidateModels
} from "../wgs/render/LostContextRecovery";
import {
    RenderFlags
} from "../wgs/scene/RenderFlags";
import * as shadow from "../wgs/render/ShadowMap";
import {
    GroundFlags
} from "../wgs/render/GroundFlags";
import {
    ResetFlags
} from "../wgs/scene/ResetFlags";
import {
    GroundReflection
} from "../wgs/render/GroundReflection";
import {
    ModelLayers
} from "./ModelLayers";
import {
...
} from "../wgs/render/DecodeEnvMap";
import {
    getResourceUrl
} from "../globals";
import {
    SAOShader
} from "../wgs/render/SAOShader";
import {
    VBIntersector
} from "../wgs/scene/VBIntersector";
import {
    VertexBufferReader
} from "../wgs/scene/VertexBufferReader";
import {
    BoundsCallback
} from "../wgs/scene/BoundsCallback";
...
import {
    SelectionType
} from "../tools/SelectionType";
import {
    BubbleNode
} from "./bubble";
import {
    SceneMath
} from "../wgs/scene/SceneMath";
import {
    GlobalManagerMixin
} from '../application/GlobalManagerMixin';
import {
    Prefs3D,
    Prefs2D,
    Prefs
} from "./PreferenceNames";
import SheetRenderContext from "../wgs/render/SheetRenderContext";
import {
    LMVRenderer
} from "../wgs/render/LMVRenderer";
...
export function Viewer3DImpl(thecanvas, theapi) {
    ...
    var _reqid, _needsResize, _newWidth, _newHeight, _materials, _renderLoopOn;
    ...
    var isRightBtnSelectionEnabled = false;
    ...
    var _onLoaderRepaintRequest = (e) => {
        if (!e ? .model || _renderScene.findModel(e.model.id)) {
            // only redraw if the source of this event is unknown or visible
    ...
    var _cutPlaneSets = {};
    // Global toggle which skips rendering any fragment inside the smallest convex shape generated by the cutplanes
    // Applies to all cutplane sets at once. Used primarily for inverted section box
    var _cutPlaneHideInterior = false;
    ...
    // once we gained confidence in deferred consolidation, we can remove this flag together with parallel consolidation
    this.useDeferredConsolidation = false;
    ...
    this.initialize = function(initOptions = {}) {
        ...
        this.useDeferredConsolidation = initOptions && !!initOptions.useDeferredConsolidation;
        ...
        this.api.addEventListener(et.LOADER_REPAINT_REQUEST_EVENT, _onLoaderRepaintRequest);
    };
    ...
    this.get2DModels = function() {
        return _renderScene.getModels().filter(m => m.is2d());
    };

    this.get3DModels = function() {
        return _renderScene.getModels().filter(m => m.is3d());
    };
    ...
    function updateCanvasSize(noEvent) {
        ...
        // Additionally update the device pixel ratio here, in case device pixel ratio changes are not
        // properly detected. This happens, for example, when window.devicePixelRatio is set directly.
        const devicePixelRatio = _this.getWindow().devicePixelRatio;
        if (_this.glrenderer().getPixelRatio() !== devicePixelRatio) {
            _this.glrenderer().setPixelRatio(devicePixelRatio);
        }
    ...
    function updateGroundTransform() {
        ...
        if (_this.model && !_this.model.isLoadDone()) {
            ...
            groundBox = _this.getVisibleBounds(true, false, undefined, undefined);
        }
    ...
    var _screenDrawnFlags = {
        NOTHING_DRAWN: 0,
        MODEL_DRAWN: 1 << 0,
        BACKGROUND_DRAWN: 1 << 1,
        OVERLAY_DRAWN: 1 << 2,
        REFLECTION_DRAWN: 1 << 3,
        ALL_DRAWN: (1 << 4) - 1
    };
    ...
    function cmdUpdateShadowMap() {
        ...
        if (_shadowMaps.state === shadow.SHADOWMAP_NEEDS_UPDATE) {
            ...
            _rcs.frameRemaining = _shadowMaps.startUpdate(_renderScene, _rcs.frameRemaining, _this.camera, _shadowLightDir, _materials);

        } else if (_shadowMaps.state == shadow.SHADOWMAP_INCOMPLETE) {

            // continue shadow map update as long as we have time
            _rcs.frameRemaining = _shadowMaps.continueUpdate(_renderScene, _rcs.frameRemaining, _materials);
    ...
    function cmdBeginPhase() {
        ...
        _renderScene.reset(_this.camera, _rcs.phase, _rcs.getParam("BeginPhase.moved"), _materials.getCutPlanes(), _cutPlaneHideInterior);

        return false;
    }

    function cmdMainRender() {
        if (!_renderScene.isEmpty() && !_renderScene.isDone()) {
            ...
            _rcs.frameRemaining = _renderScene.renderSome(renderSomeCallback, _rcs.frameRemaining);

            // TODO - cmdMainRender gets used by a number of systems - what sort of progress should really happen here?
            if (_rcs.signalProgressByRendering) {
                _this.signalProgress(100.0 * _renderScene.getRenderProgress(), ProgressState.RENDERING);
                if (ENABLE_DEBUG_RCS) {
                    console.log("  %%% percent done " + (100 * _renderScene.getRenderProgress()));
                }
            }
        }
        // if there is time left, continue on (return false), else return true, which means "stop for now"
        return !_renderScene.isDone();
    }
    ...
    function cmdRenderOverlays() {
        ...
        if (_rcs.drawOverlay) {
            ...
            if ((!_renderScene.isEmpty() && _renderScene.isDone()) || _this.showOverlaysWhileMoving) {
    ...
    function cmdPostAndPresent() {
        ...
        if (_rcs.screenDrawn &&
            ...
            _this.api.dispatchEvent({
                type: et.RENDER_PRESENTED_EVENT
            });
        }
    ...
    this.tick = function(highResTimeStamp) {
        ...
        // it comes in about 100 ticks later than whatever number we get when we call performance.now() right here.
        // TODO how's that work? Why this mis-sync?

        // In the very first render loop, 'highResTimeStamp' is not defined yet,
        // so we catch that case setting it to 0.
        if (!highResTimeStamp) {
            highResTimeStamp = 0;
        }
        ...
        const animationMoved = updateAnimations(highResTimeStamp);

        // update controls (e.g. view cube, home button, orbit, key press) and see if that has affected the view
        const controlsMoved = _this.controls.update(highResTimeStamp);

        // see if more data was loaded.
        const sceneChanged = _renderScene && _renderScene.update(highResTimeStamp);


        const moved = controlsMoved || animationMoved || _cameraUpdated || sceneChanged;
        ...
        var lastFrameValid = _renderScene.frameResumePossible();
        ...
        if (_needsClear || _needsRender || _overlayDirty || _immediateSilentRender) {
            ...
            if (_needsClear || _needsRender || _immediateSilentRender) {
                ...
                if (_renderScene) {
                    ...
                    if (_renderScene.hasHighlighted()) {
                        // set phase and reset
                    ...
                    if (!_renderScene.areAllVisible() && _this.showGhosting) {
                ...
                if ((_rcs.renderType === _rcs.RENDERMODE_PROGRESSIVE) && _renderScene) {
        ...
        if (_workThisTick !== _workPreviousTick) {
            if (!_workThisTick) {
                _this.glrenderer().notifyFinalFrameRendered();
            }
            _this.api.dispatchEvent({
                type: et.FINAL_FRAME_RENDERED_CHANGED_EVENT,
                value: {
    ...
    this.run = function() {
        ...
        setTimeout(function() {
            ...
            // Edge case: Render loop was already stopped again.
            if (!_renderLoopOn) {
                return;
            }
    ...
    this.updateClearColors = function() {
        let clearColorTop = this.clearColorTop;
        let clearColorBottom = this.clearColorBottom;

        // apply black/white swap to clear color if wanted
        if (this.is2d && this.swapBlackAndWhite) {
            const isWhite = (c) => c.x === 1 && c.y === 1 && c.z === 1;
            const isBlack = (c) => c.x === 0 && c.y === 0 && c.z === 0;
            const swapIfNeeded = (c) => isWhite(c) ? new THREE.Vector3(0, 0, 0) : (isBlack(c) ? new THREE.Vector3(1, 1, 1) : c);

            clearColorTop = swapIfNeeded(clearColorTop);
            clearColorBottom = clearColorBottom && swapIfNeeded(clearColorBottom);
        }

        _renderer.setClearColors(clearColorTop, clearColorBottom);
    };
    ...
    this.setRightBtnSelection = function(enable) {
        isRightBtnSelectionEnabled = enable;
    };

    this.isRightBtnSelectionEnabled = function() {
        return isRightBtnSelectionEnabled;
    };
    ...
    function updateSelectionProxies() {
        for (var id in _this.selectionMeshes) {
            ...
            if (proxy.model && proxy.model.isConsolidated()) {
                ...
                // if its geometry is not valid anymore, remove the fragment from the selection
                if (!proxy.geometry)
                    _this.highlightFragment(proxy.model, proxy.fragId, false);
            }
        }
    }

    this.getOverlayScenesToRender = function() {
        const scenes = {};
        for (const [name, scene] of Object.entries(this.overlayScenes)) {
            if (!scene.hiddenFromMainRender) {
                scenes[name] = scene;
            }
        }
        return scenes;
    ...
    this.renderOverlays = function() {
        ...
        const overlayScenes = this.getOverlayScenesToRender();
        _renderer.renderOverlays(overlayScenes, this.lightsOn ? this.lights : this.no_lights);
    ...
    var setLights = function(amb_light, dir_light1, state, isForOverlay) {
        ...
        ac = ac ? .slice(0) ? ? _defaultAmbientColor.toArray(); // create copy to not change preset!
        dc = dc ? .slice(0) ? ? _defaultDirLightColor.toArray(); // create copy to not change preset!
    ...
    this.setViewFromCamera = function(camera, skipTransition, useExactCamera) {
        ...
        const cameraDirection = camera.target.clone()
            .sub(camera.position)
            .normalize();

        const isTopDown = cameraDirection.distanceTo(worldUp.clone().negate()) < 1E-6;

        if (useExactCamera) {
            if (this.api.prefs)
                this.api.prefs.set('fusionOrbitConstrained', worldUp.equals(camera.up));
        } else if (!isTopDown) {
            // Only apply camera up as world up if it's not the default view, which is looking downward
            camera.up = worldUp;
        }
        ...
        if (navapi) {
            ...
            if (!skipTransition) {
                ...
                navapi.setWorldUpVector((useExactCamera || isTopDown) ? worldUp : tc.up);
    ...
    this.setUp2DMode = function(model, isOverlay) {
        ...
        const idMatName = _materials.create2DMaterial(null, {}, true, false, function() {
    ...
    this.addModel = function(model, preserveTools) {
        ...
        // Fire an event for the addition of a model into the _renderScene
        this.api.fireEvent({
            type: et.MODEL_ADDED_EVENT,
            model,
            preserveTools,
            isOverlay
        });
    ...
    this.setupMesh = function(model, threegeom, materialId, matrix) {
        ...
        m.isLine = threegeom.isLines;
        m.isWideLine = threegeom.isWideLines;
        m.isPoint = threegeom.isPoints;
        m.is2d = threegeom.is2d;
    ...
    this.selection2dOverlayName = function(model) {
    ...
    this.init2dSelection = function(model) {
        ...
        var overlayName = this.selection2dOverlayName(model);
    ...
    this.onLoadComplete = function(model) {
        ...
        this.signalProgress(100, ProgressState.LOADING, model);
    ...
    this.unloadModel = function(model, keepResources) {
        ...
        this.api.dispatchEvent({
            type: et.BEFORE_MODEL_UNLOAD_EVENT,
            model: model
        });
        ...
        if (!this.removeModel(model) && !_renderScene.removeHiddenModel(model)) {
        ...
        if (model.is2d() && this.overlayScenes[model.id]) {
            this.removeOverlayScene(this.selection2dOverlayName(model));
        }

        this.api.dispatchEvent({
            type: et.MODEL_UNLOADED_EVENT,
            model: model
        });
    };
    ...
    this.removeModel = function(model) {
        ...
        if (!_renderScene.removeModel(model)) {
            return false;
        }
        ...
        if (model === this.model) {
            ...
            if (!_renderScene.isEmpty())
                this.model = _renderScene.getModels()[0];
        }
    ...
    function syncIdTargetCount() {
        ...
        var sceneModelCount = _renderScene.getModels().length;
    ...
    this.unloadCurrentModel = function() {
        ...
        if (this.model) {
            ...
            if (this.is2d) {
                this.is2d = undefined;
                this.removeOverlayScene(this.selection2dOverlayName(this.model));
                _renderer.exit2DMode();
            } else {
        ...
        var models = _renderScene.getAllModels();
    ...
    var setupSelectionHighlight = function() {
        ...
        _this.selectionMaterialBase = new THREE.MeshPhongMaterial({
            specular: 0x080808,
            opacity: 1.0,
            transparent: false
        });
        _this.selectionMaterialTop = new THREE.MeshPhongMaterial({
            specular: 0x080808,
            opacity: 0.15,
            transparent: true
        });
    ...
    this.createOverlayScene = function(name, materialPre, materialPost, camera, needIdTarget = false, needSeparateDepth = false, hiddenFromMainRender = false) {
        if (materialPre) {
    ...
    this.addOverlay = function(overlayName, mesh) {
        if (this.overlayScenes[overlayName] && mesh) {
    ...
    this.removeOverlay = function(overlayName, mesh, disposeGeometry = false) {
        if (this.overlayScenes[overlayName] && mesh) {
    ...
    this.removeMultipleOverlays = function(overlayName, meshes, disposeGeometry = false) {
        for (var i in meshes) {
            if (!Object.prototype.hasOwnProperty.call(meshes, i)) continue;
            this.removeOverlay(overlayName, meshes[i], disposeGeometry);
        }
    };

    this.clearOverlay = function(overlayName, disposeGeometry = false) {
    ...
    this.getVisibleBounds = function(includeGhosted, includeOverlays, bboxFilter, excludeShadow) {
        ...
        if (!_renderScene.isEmpty()) {
            ...
            result = _renderScene.getVisibleBounds(includeGhosted, bboxFilter, excludeShadow).union(result);
    ...
    this.isWholeModelVisible = function() {
        return _renderScene ? _renderScene.areAllVisible() : true;
    };
    ...
    this.highlightFragment = function(model, fragId, value, simpleHighlight) {
        ...
        if (useOverlay) {
            ...
            if (model.is2d()) overlayName = this.selection2dOverlayName(model);
    ...
    this.explode = function(scale, options = {}) {

        const optionKeys = Object.keys(options);

        for (let key of optionKeys) {
            if (!Object.prototype.hasOwnProperty.call(_explodeOptions, key))
                logger.warn(`Property "${key}" is not part of Explode options.`);
        }

        const optionsUnchanged = (() => {
            for (let key of optionKeys) {
                if (Object.prototype.hasOwnProperty.call(_explodeOptions, key) &&
                    options[key] != _explodeOptions[key])
                    return false;
            }
            return true;
        })();
        ...
        scale = Number(scale);
        if (scale == _explodeScale && optionsUnchanged)
            return false;

        _explodeScale = scale;

        // apply new options values to _explodeOptions
        // disallow adding unspecified property names to ensure consistency of viewerState
        optionKeys
            .filter(key => Object.prototype.hasOwnProperty.call(_explodeOptions, key))
            .forEach(key => _explodeOptions[key] = options[key]);

        this.refreshExplode();

        this.api.dispatchEvent({
            type: et.EXPLODE_CHANGE_EVENT,
            scale: scale,
            options: options
        });
        return true;
    ...
    this.refreshExplode = function() {
        ...
        _renderScene.explode(_explodeScale, _explodeOptions);
    ...
    /**
     * Gets a copy of the last applied explode options
     */
    this.getExplodeOptions = function() {
        return { ..._explodeOptions
        };
    };
    ...
    this.lockExplode = function(dbids, lock, model) {
        ...
        if (changed && _explodeScale > 0) {
            _renderScene.explode(_explodeScale);
            this.sceneUpdated(true);
        }
        return changed;
    };
    ...
    this.setLightPreset = function(index, force, callback) {
        ...
        const dwfModels = _renderScene ? .getModels().filter((model) => {
    ...
    this.dtor = function() {
        ...
        this.api.removeEventListener(et.LOADER_REPAINT_REQUEST_EVENT, _onLoaderRepaintRequest);
    ...
    this.hideLines = function(hide) {
        if (_renderScene && !_renderScene.isEmpty()) {
            _renderScene.hideLines(hide);
            this.sceneUpdated(true);
        }
    };
    ...
    this.setCutplanesHideInterior = function(enabled) {
        _cutPlaneHideInterior = Boolean(enabled);
    };

    this.cutplanesHideInteriorEnabled = function() {
        return _cutPlaneHideInterior;
    };
    ...
    this.setCutPlanesInScene = function(planes) {
        _renderer.toggleTwoSided(_materials.setCutPlanes(planes, _cutPlaneHideInterior ? 1 : 0));
        this.sceneUpdated();
    };
    ...
    this.setCutPlanes = function(planes) {
        ...
        this.api.dispatchEvent({
            type: et.CUTPLANES_CHANGE_EVENT,
            planes: planes
        });
    };
    ...
    this.fireRenderOptionChanged = function() {
        ...
        this.api.dispatchEvent({
            type: et.RENDER_OPTION_CHANGED_EVENT
        });
    };
    ...
    /**
     *
     * @param {THREE.Ray} ray               - Ray to intersect from
     ...
     * @param {Object} [options]            - Optional Rayintersection options:
     * @param {function} [options.filter]           - Filter function (hitResult) => bool. (see VBIntersector for hitresult content)
     * @param {bool}     [options.ignore2dModelsOn3d]   - Whether to ignore 2d models when in 3d mode.
    ...
    this.rayIntersect = function(ray, ignoreTransparent, dbIds, modelIds, intersections, options = {}) {
        ...
        if (options.ignore2dModelsOn3d) {
            const model3DIds = this.get3DModels().map(m => m.id);
            modelIds = modelIds ? modelIds.filter(id => model3DIds.includes(id)) : model3DIds;
        }

        var result = _renderScene.rayIntersect(ray.origin, ray.direction, ignoreTransparent, dbIds, modelIds, intersections, getDbIdAtPointFor2D, options);
    ...
    this._2dHitTestViewport = function(vpVec, searchRadius, minPixelId) {
        ...
        const model = _renderScene.findModel(_idRes[1]) || this.model;
        if (!model)
            return null;
    ...
    this.hitTestViewport = function(vpVec, ignoreTransparent, dbIds, modelIds, intersections, options) {
        let result;
        ...
        if (this.is2d) {
            ...
            result = this.castRayViewport(vpVec, ignoreTransparent, dbIds, modelIds, intersections, options);
        }

        return result;
    };

    this.hitTest = function(clientX, clientY, ignoreTransparent, dbIds, modelIds, options) {

        return _this.hitTestViewport(this.clientToViewport(clientX, clientY), ignoreTransparent, dbIds, modelIds, undefined, options);
    };
    ...
    this.hitBoxTestViewport = function(vpVec, widthRatio, heightRatio) {
        ...
        for (let i = 0; i < ids.length; i++) {
            const model = _renderScene.findModel(ids[i][1]) || this.model;
            if (model) {
    ...
    this.cancelLeafletScreenshot = function() {
        _this.api.dispatchEvent({
            type: et.CANCEL_LEAFLET_SCREENSHOT
        });
    };
    ...
    this.geomCache = function() {
        ...
        if (!_geomCache) {
            this.setGeomCache(new OtgResourceCache(_this.api.config));
        }
    ...
    this.sceneUpdated = function(objectsMoved, skipRepaint, skipFireEvent) {

        this.invalidate(!skipRepaint, false, !skipRepaint);

        // Mark the scene bounds for update
        if (_renderScene && objectsMoved) {
            _renderScene.invalidateVisibleBounds();
            this.zoomBoundsChanged = true;
        }
    ...
    this.clientToWorld = function(clientX, clientY, ignoreTransparent, ignore2dModelBounds, ignore2dModelsOn3d) {
        ...
        if (this.is2d) {
            ...
            if (collision) {
                collision.z = 0;
                if (ignore2dModelBounds || modelData.hidePaper || modelData.bbox.containsPoint(collision)) {
                    result = {
                        point: collision,
                        model: model
                    };
                ...
                result = {
                    point: this.camera.position.clone(),
                    model
                };
            }
        } else {
            // hitTest handles multiple scenes
            result = this.hitTest(clientX, clientY, ignoreTransparent, undefined, undefined, {
                ignore2dModelsOn3d
            });
            if (result) {
    ...
    this.findModel = function(value, includeHidden) {
        ...
        let model = _renderScene.getModels().find(filter);

        // Optional: Search hidden models
        if (includeHidden && !model) {
            model = _renderScene.getHiddenModels().find(filter);
        }
    ...
    this.showTransparencyWhenMoving = function(enabled) {
        _renderScene.enableNonResumableFrames = enabled;
    };
    ...
    this.fitToView = function(aggregateSelection, immediate) {
        ...
        if (aggregateSelection.length === 0) {
            ...
            var allModels = _renderScene.getModels();
        ...
        if (_renderScene.getModels().length === 1) {
    ...
    this.onModelTransformChanged = function(model) {
        ...
        this.api.fireEvent({
            type: et.MODEL_TRANSFORM_CHANGED_EVENT,
            model,
            matrix: model.getModelTransform()
        });
    };
    ...
    this.setPlacementTransform = function(model, matrix) {
        ...
        this.api.fireEvent({
            type: et.MODEL_PLACEMENT_CHANGED_EVENT,
            model,
    ...
    function find2DLayerBounds(model, fragId, visibleLayerIds, bc) {
        ...
        var vbr = new VertexBufferReader(geometry);
    ...
    this.onWebGLcontextLost = function() {
        ...
        this.stop();
        this.api.fireEvent({
            type: et.WEBGL_CONTEXT_LOST_EVENT
        });
    };
     ...
     * Note this function may not work well with transparency cases.
     * @private
     */
    this.handleInitialVisibility = function(model) {
        logger.info("Hiding objects that are marked as 'hidden'. This can take up to minutes depending on model size ...");
        ...
        function hideInvisibleNodes(instanceTree) {
            ...
            logger.info("Hiding " + invisibleNodes.length + " objects ...");

            let progress = 0;
            if (invisibleNodes.length > 0) {
                const lastIndex = invisibleNodes.length - 1;
                for (let i = 0; i < invisibleNodes.length; ++i) {
                    that.visibilityManager.setVisibilityOnNode(invisibleNodes[i], false, model, i !== lastIndex, false);

                    progress = Math.round(i / invisibleNodes.length * 100);
            ...
            logger.info("Done hiding objects.");
        }

        // Delay hiding invisible nodes to allow other instance tree handlers to execute first
        model.getObjectTree(instanceTree => setTimeout(() => hideInvisibleNodes(instanceTree)));
    };
    ...
    this.consolidateModel = function(model, byteLimit) {
        model.consolidate(_materials, byteLimit, _webglrender, this.useDeferredConsolidation);
    };
    ...
    this.setViewportBounds = function(model, bounds) {
        ...
        this.api.fireEvent({
            type: et.MODEL_VIEWPORT_BOUNDS_CHANGED_EVENT,
            model,
            bounds
        });
    };
