 ...
 *
 * 1. __Facilitate use in react__: Just set an array of nodes from React property and AggregatedView takes care to make sure that models are loaded and shown/hidden as needed.
 * 2. __LRU-Caching of models__: Keep models in memory for fast switches, but unload unused models if memory is critical
...
export class AggregatedView {
    ...
    constructor() {
        ...
        av.getGlobal().LMV_MAIN_VIEW = this;
    ...
    destroy() {
        ...
        av.getGlobal().LMV_MAIN_VIEW = null;
    }
    ...
    _initForEnvironmentChange(bubbleNode, isShowing) {
        if (this.areAllNodes2D() && bubbleNode.is3D()) {
            ...
            this.fireEvent({
                type: Events.ENVIRONMENT_CHANGED,
                is3D: this.is3D
            });
        }
    }
    ...
    load(bubbleNode, customLoadOptions = null) {
        ...
        // return the existing load promise or the model itself
        ...
        if (item.error) {
            return Promise.reject('Model load failed already');
        }
        ...
        if (bubbleNode.is3D()) {
            ...
            if (this.waitForFirstModel) {
                ...
                return new Promise((resolve, reject) => {
                    this.loadPendingPromises[modelKey] = {
                        resolve,
                        reject
                    };
                });
        ...
        const loadOptions = {
            ...
            // Optional: Avoid applying 'nwModelToWorldTransform' at load-time. 
            // If this is false, a custom rotation from NWC metadata is applied if available.
            // avoidNwcRotation: true,
        ...
        item.loadingPromise = new Promise((resolve, reject) => {
            ...
            this._applyAlignmentService(bubbleNode, loadOptions).then(() => {
                if (!this.viewer) { // Check if viewer was unloaded
                    reject();
                }

                return this.viewer.loadDocumentNode(doc, bubbleNode, loadOptions);
            }).then((model) => {
                if (!this.viewer) { // Check if viewer was unloaded
                    reject();
                }
    ...
    setNodes(bubbleNodes, diffConfig) {
        ...
        bubbleNodes.forEach(node => {
            newModelKeys[node.getModelKey()] = true;
        });
    ...
    _consolidateVisibleModels() {
        ...
        // Skip consolidation on mobile due to more limited memory on weaker devices, unless deferred consolidation is enabled (which reduces mem spikes on load)
        if (isMobileDevice() && !this.viewer.impl.useDeferredConsolidation)
            return;
        ...
        for (let i = 0; i < visibleModels.length; i++) {
            ...
            // Skip anything 2D (sheets/leaflets). Skip models from scene builder
            if (model.is2d() || model.isSceneBuilder()) {
                return;
            }
    ...
    _updateRefPoint(bubbleNode) {
        ...
        // If we encounter such a scenario, we just skip updating global offset here
        ...
        let tf = aecModelData.refPointTransformation; // Matrix4x3 as array[12]
        if (!tf) {
            return;
        }
        ...
        const refPoint = {
            ...
            y: tf[10],
            z: 0.0
        }; // refPoint = refPointTransform * (0,0,0)
        ...
        if (this.isUsingAlignmentService()) {
            ...
            // In this case, we cannot say where the model is placed, so we have to stop here.
    ...
    _updateGhosting() {
        ...
        // make sure the viewer is refreshed after updating ghosting,
        // otherwise leaving the diff view will leave models in ghosted state
    ...
    async _initAlignmentService() {
        ...
        const initService = async () => {
            ...
            if (!this.viewer) { // Check viewer wasn't closed in the meantime
                return;
            }
    ...
    async _applyAlignmentService(bubbleNode, loadOptions) {
        ...
        track('viewer.modelalignment.alignment_loaded', {
            hasAlignmentTransform: Boolean(matrix)
        });
    ...
    _checkAlignmentService() {
        ...
        this.fireEvent({
            type: Events.ALIGNMENT_SERVICE_FAILED,
            message
        });
