...
export function ViewerState(viewer) {
    ...
    this.createObjectSets = function(viewerState) {
        ...
        for (var i = 0; i < visibleModels.length; i++) {
            ...
            if (!model.is2d()) {
                ...
                objectSet[i].explodeOptions = viewer.getExplodeOptions();
            } else {
    ...
    this.getState = function(filter) {
        ...
        viewerState["autocam"] = {
            sceneUpDirection: viewer.autocam.sceneUpDirection.clone(),
            sceneFrontDirection: viewer.autocam.sceneFrontDirection.clone(),
            cubeFront: viewer.autocam.cubeFront.clone()
        };
    ...
    this.restoreObjectSet = function(viewerState) {
        ...
        for (var i = 0; i < objectSets.length; ++i) {
            ...
            if ("explodeScale" in objectSet) {
                ...
                var explodeOptions = objectSet.explodeOptions;
                if (viewer.explode) {
                    viewer.explode(explodeScale, explodeOptions);
                }
            }
        }
        ...
        if (isolateAggregate.length > 0) {
            viewer.impl.visibilityManager.aggregateIsolate(isolateAggregate, {
                hideLoadedModels: false
            });
        }
    ...
    this.restoreState = function(viewerState, filter, immediate) {
        ...
        var nav = viewer.navigation;
        var is3d = !viewer.impl.is2d;

        // Objectset
        this.restoreObjectSet(viewerState);

        const autocamState = viewerState.autocam;
        if (autocamState) {
            const autocam = viewer.autocam;
            autocam.sceneUpDirection.copy(autocamState.sceneUpDirection);
            autocam.sceneFrontDirection.copy(autocamState.sceneFrontDirection);
            autocam.cubeFront.copy(autocamState.cubeFront);
            autocam.camera.worldup.copy(autocamState.sceneUpDirection);
            autocam.cameraChangedCallback(true);
        }
        ...
        if (viewport) {
            ...
            if (!worldUp) {
                ...
                const upVectorArray = model ? .getUpVector();
                if (upVectorArray) {
        ...
        if (renderOptions) {
            ...
            if ("environment" in renderOptions) {
                ...
                if (lightPresetIndex !== -1 && lightPresetIndex !== prefs.get('lightPreset') && is3d) {
            ...
            if (appearance) {
                ...
                if (("ghostHidden" in appearance) && appearance.ghostHidden !== prefs.get('ghosting')) {
                    is3d && viewer.setGhosting(appearance.ghostHidden);
                }
            ...
            if (is3d && saoEnabled !== prefs.get('ambientShadows') && antiAliasing !== prefs.get('antialiasing')) {
                viewer.setQualityLevel(saoEnabled, antiAliasing);
            }
        }

        // Restore cutplanes (aka: Sectioning) data only for 3d models.
        if (Array.isArray(viewerState.cutplanes) && is3d) {
            var cutplanes = [];
    ...
    this.areEqual = function(viewerStateA, viewerStateB, filter) {
        ...
        if (
            ...
            !areNumbersEqual(objectA.explodeOptions ? .magnitude, objectB.explodeOptions ? .magnitude, epsilon) ||
            !areNumbersEqual(objectA.explodeOptions ? .depthDampening, objectB.explodeOptions ? .depthDampening, epsilon) ||
