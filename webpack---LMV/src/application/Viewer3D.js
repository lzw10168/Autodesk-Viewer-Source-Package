...
import {
    displayUnitsEnum,
    displayUnitsPrecisionEnum
} from '../../src/measurement/DisplayUnits';
import {
    ProfileManager
} from './ProfileManager';
import {
...
import {
    errorCodeString,
    ErrorCodes
} from "../file-loaders/net/ErrorCodes";
import {
    EventDispatcher
...
import {
    ScreenModeMixin,
    ScreenMode
} from "./ScreenModeDelegate";
import {
    ExtensionMixin
} from "./ExtensionManager";
import {
    HotkeyManager
} from "../tools/HotkeyManager";
import {
    isNodeJS
} from "../compat";
import {
...
import {
    ViewingUtilities
} from "../tools/ViewingUtilities";
import {
    DefaultHandler
} from "../tools/DefaultHandler";
import {
    GestureHandler
} from "../tools/GestureHandler";
import i18n from "i18next";
import * as et from "./EventTypes";
import {
    FileLoaderManager
} from "./FileLoaderManager";
import {
    clearPropertyWorkerCache
} from "../file-loaders/main/PropDbLoader";
import {
    ViewerState
} from "./ViewerState";
import {
    OrbitDollyPanTool
} from "../tools/OrbitDollyPanTool";
import {
    Autocam
} from "../tools/autocam/Autocam";
import {
    Viewer3DImpl,
...
import {
    Consolidation
} from "../wgs/scene/consolidation/Consolidation";
import {
    KeyCode
} from "../tools/KeyCode";
import {
    ForgeLogoSpinner
} from './ForgeLogoSpinner';
import {
    LoadingSpinner
} from './LoadingSpinner';
import {
...
import {
    GlobalManagerMixin
} from './GlobalManagerMixin';
import {
    LightPresets
} from "./LightPresets";
import {
    ModelExploder
} from "../wgs/scene/ModelExploder";
import {
    analytics
} from '../analytics';
import {
 ...
 * It contains everything that is needed to connect to the Autodesk Platform Services and display 2D and 3D models.
...
export function Viewer3D(container, config) {
    ...
    if (container) {
        ...
        // LMV-5628: Enable the APS Logo in the bottom right of the canvas.
        if (!isPhoneFormFactor() && !av.Private.DISABLE_FORGE_LOGO && !av.Private.DISABLE_FORGE_CANVAS_LOGO) {
            ...
            img.src = Autodesk.Viewing.Private.getResourceUrl('res/ui/powered-by-autodesk-oneline-blk-rgb.png');
...
Object.defineProperty(Viewer3D.prototype, 'sceneAfter', {
    get: function() {
        return this.impl.sceneAfter;
    },
    set: function() {
        throw "Do not set viewer.sceneAfter";
    }
});
...
Viewer3D.prototype.registerUniversalHotkeys = function() {
    ...
    */
    ...
    self._hotkeyManager.pushHotkeys("Autodesk.Pan", hotkeys, {
...
Viewer3D.prototype.initialize = function(initOptions) {
    ...
    if (isIOSDevice() && this.getWindow().devicePixelRatio) {
        /** @todo test if this can be removed (and if not, test whether the pixel ratio is still correct) */
    ...
    this.addWindowEventListener('resize', this.onResizeCallback);


    this.onPixelRatioChanged = () => {
        if (!self.impl ? .glrenderer()) {
            return;
        }

        const pixelRatio = self.getWindow().devicePixelRatio;
        matchMedia(`(resolution: ${pixelRatio}dppx)`)
            .addEventListener('change', self.onPixelRatioChanged, {
                once: true
            });

        self.impl.glrenderer().setPixelRatio(pixelRatio);
        self.resize();
    };
    this.onPixelRatioChanged();
    ...
    this.addEventListener(et.GEOMETRY_LOADED_EVENT, function(e) {
        ...
        const geometryList = e.model.getGeometryList();

        // @todo: a 'has edge?' query should not be based on custom state/type guessing but part of the
        // geometry class (type or property) instead.
        // @todo: this is a weird location to add some state/type information to the displayed model.
        // Consider moving this to some coordinated and consolidated geometry post processing step or make
        // it part of geometry creation. But the viewer should not be concerned with such details on a event-level basis.
        e.model.hasEdges = false;
        for (let i = 0; i < geometryList ? .getCount(); i++) { // geometryList might not exist if the model was already destroyed.
            const geom = geometryList.getGeometry(i);
            if (!geom) {
                continue;
            }
            if (geom.isLine || geom.iblines) {
    ...
    this.addEventListener(et.FRAGMENTS_LOADED_EVENT, function(e) {
        if (e.model.isConsolidated()) {
            e.model.unconsolidate(true, true);
            this.impl.consolidateModel(e.model);
        }

        this.impl.invalidate(true, true);
    });

    this.addEventListener(et.FRAGMENTS_UNLOADED_EVENT, function(e) {
        if (e.model.isConsolidated()) {
            e.model.unconsolidate(true, true);
            this.impl.consolidateModel(e.model);
        } else {
            // Memory on the GPU potentially got freed up, so reevaluate memory usage
            e.model.chooseMemoryTypes();
        }
...
Viewer3D.prototype.setUp = function(config) {
    ...
    const isFedramp = this.config.region ? .toLowerCase() === 'fedramp';
    if (!isFedramp) {
...
Viewer3D.prototype.__initAutoCam = function(impl) {
    ...
    function onTransitionCompleted() {
        self.fireEvent({
            type: et.CAMERA_TRANSITION_COMPLETED
        });
    }
...
Viewer3D.prototype.registerDimensionSpecificHotkeys = function(is2d) {
    ...
    if (is2d) {
        ...
        this._hotkeyManager.pushHotkeys("Autodesk.Orbit", hotkeys, {
            tryUntilSuccess: true
        });
    }
};
 ...
 * @param {boolean} [options.keepCurrentModels=false] - Flag indicating whether viewer should keep or unload all other models.
...
Viewer3D.prototype.loadModel = async function(url, options, onSuccessCallback, onErrorCallback, onWorkerStart) {
    ...
    function onDone(error, model) {
        ...
        if (error) {
            self.dispatchEvent({
                type: et.LOADER_LOAD_ERROR_EVENT,
                error: error,
    ...
    if (options && options.fileLoader) {
        ...
        loader = FileLoaderManager.getFileLoaderForExtension(fileExtension, options.mimeType);
    }
    ...
    this.fireEvent({
        type: et.LOADER_LOAD_FILE_EVENT,
        loader: loaderInstance
    });
 ...
 */
Viewer3D.prototype.isLoadDone = function({
    geometry = true,
    propDb = true,
    textures = true,
    hidden = false,
...
Viewer3D.prototype.waitForLoadDone = function(include) {
    return new Promise((resolve, reject) => {
        ...
        var {
            geometry = true, propDb = true, textures = true, hidden = false, onlyModels = undefined
        } = include || {};
        ...
        this.addEventListener(et.MODEL_ROOT_LOADED_EVENT, onEvent, {
            priority: -1000000
        });
 ...
 */
Viewer3D.prototype.loadDocumentNode = function(avDocument, manifest, options = {}) {
    const leafletOptions = options.leafletOptions || {};
    let modelRootUrl = avDocument.getViewableUrn(manifest, leafletOptions);
    let sharedDbPath = manifest.findPropertyDbPath();

    modelRootUrl = avDocument.getFullPath(modelRootUrl);
    sharedDbPath = avDocument.getFullPath(sharedDbPath);

    const acmSessionId = avDocument.getAcmSessionId(modelRootUrl);
    const loadOptions = {
    ...
    const geomNode = manifest.findParentGeom2Dor3D();
    const modelExtensions = geomNode ? .extensions();
    ...
    const res = leafletOptions.isPdf ? this.loadExtension("Autodesk.PDF", this.config) : Promise.resolve();

    const useUnderlayRaster = !options.disableUnderlayRaster && leafletOptions.tempRasterPath;
    const tempRasterPromise = useUnderlayRaster ? this._loadTempRasterModel(avDocument, options, manifest) : Promise.resolve();

    return res.then(() => tempRasterPromise)
        .then((underlayModel) => new Promise((resolve, reject) => {
            if (useUnderlayRaster && underlayModel && underlayModel !== this.getUnderlayRaster(manifest)) {
            ...
            this.loadModel(modelRootUrl, options,
                model => {
                    ...
                    const keepView = options.loadAsHidden || options.preserveView;
                    if (!keepView && manifest.type() === 'view') {
                        this.setView(manifest, {
                            skipTransition: true
                        });
                    }
...
Viewer3D.prototype.restoreState = function(viewerState, filter, immediate) {
    ...
    if (success) {
        this.dispatchEvent({
            type: et.VIEWER_STATE_RESTORED_EVENT,
            value: success
        });
    }
    return success;
...
Viewer3D.prototype.getViewpointCamData = function(viewpointCamera) {
    const camPosition = viewpointCamera.position;
    const camTarget = viewpointCamera.target;
    const camUp = viewpointCamera.up;
    const isPerspective = viewpointCamera.isPerspective ? 0 : 1;

    const camData = [camPosition.x, camPosition.y, camPosition.z,
        camTarget.x, camTarget.y, camTarget.z,
        camUp.x, camUp.y, camUp.z,
        viewpointCamera.aspect, viewpointCamera.fov * (Math.PI / 180), viewpointCamera.orthoScale, isPerspective
    ];
    return camData;
};
 ...
 * @param {boolean} [options.skipViewpointExtra=false] - true to skip using extra viewpoint information
 *
...
Viewer3D.prototype.setView = function(viewNode, options) {
    ...
    // 3D models
    let camera;
    let svf = model.loader ? .svf;
    let viewpoint = viewNode.data.isViewpoint && svf ? svf.viewpoints ? .[viewNode.data.entry] : null;
    let camData = viewNode.data.camera;
    let isOtg = model.isOTG();
    let modelConsolidated = model.isConsolidated();
    if (!skipViewpointExtra && viewpoint) {
        const viewpointCamera = viewpoint.camera;
        camData = this.getViewpointCamData(viewpointCamera);

        //get Overrides from loader data
        const overrideSet = svf.overrideSets[viewpoint.overrideSet];
        if (overrideSet) {
            /*
            defaultMaterialIndex - sets specific material for all dbids
            materialOverrides - sets material overrides for declared dbids
            flagOverrides - hides/shows declared dbids
            defaultFlags - hides/shows all dbids
            */
            const {
            ...
            const fragListLength = fragList.fragments.length;

            // set materials for restore
            if (!fragList.materialIdMapOriginal) {
                fragList.storeOriginalMaterials();
            }

            if (defaultMaterialIndex || materialOverrides) {
                // Unconsolidate model to apply material overrides
                if (modelConsolidated) {
                    model.unconsolidate(false, true);
                }
            }

            if (defaultMaterialIndex) {
                const materialHash = isOtg ? svf.getMaterialHash(defaultMaterialIndex) : this.impl.matman()._getMaterialHash(model, defaultMaterialIndex);
                const material = this.impl.matman()._materials[materialHash];
                if (material) {
                    for (let i = 0; i < fragListLength; i++) {
                        this.model.getFragmentList().setMaterial(i, material);
                    }
                }
            }

            // Set materials from override set
            // No overrides means to keep current visibility/materials
            // Overrides array means to restore initial state of visibility/materials
            if (materialOverrides) {
                //restore old mats first
                if (!defaultMaterialIndex) {
                    model.getFragmentList().restoreOriginalMaterials();
                }

                // Set material from material override set
                for (let i = 0; i < materialOverrides.length; i++) {
                    const dbId = materialOverrides[i].dbId;
                    const materialIndex = materialOverrides[i].materialIndex;
                    if (!Number.isFinite(dbId) || !Number.isFinite(materialIndex)) {
                        continue;
                    }
                    const materialHash = isOtg ? svf.getMaterialHash(materialIndex) :
                        this.impl.matman()._getMaterialHash(model, materialIndex);
                    const material = this.impl.matman()._materials[materialHash];
                    if (!material) {
                        continue;
                    }
            ...
            // set flagoverride (hide/show objects)
            if (defaultFlags) {
                if (defaultFlags === 1) {
                    this.hideAll();
                } else if (defaultFlags === 2) {
                    this.showAll();
                }
            }

            /**
             * flagOverrides controls if objects are visible or hidden
             * if flagOverrides has 0 length, it means we show all
             * otherwise, we iterate list and update visibility of dbids accordingly
             * `1` means hide
             * `2` means show
             */
            if (flagOverrides) {
                if (flagOverrides.length === 0) {
                    ...
                    for (let i = 0; i < flagOverrides.length; i++) {
                        const {
                            flags,
                            dbId
                        } = flagOverrides[i];

                        if (!Number.isFinite(flags) || !Number.isFinite(dbId)) {
                            continue;
                        }

                        if (flags === 1) {
                            this.hide(dbId, model);
                        } else if (flags === 2) {
            ...
            //Re-consolidate model after applying material overrides. 
            if (modelConsolidated && !model.isConsolidated()) {
                this.impl.consolidateModel(model);
                this.impl.invalidate(true, true);
            }
        }
    }

    camera = this.getCameraFromViewArray(camData, model);
    ...
    const viewNodeData = viewpoint ? ? viewNode.data;
    sp = viewNodeData.sectionPlane;
    sb = viewNodeData.sectionBox;
    sbt = viewNodeData.sectionBoxTransform;
    ...
    if (sp) {
        ...
        if (!viewNode.data.isViewpoint) {
            ...
            planes = SceneMath.box2CutPlanes(box, boxTrans);

        } else {

            const box = new THREE.Box3();
            box.set(sb.min, sb.max);
            const dim = box.max.clone().sub(box.min);

            const modelToSectionBox = (new THREE.Matrix4()).compose(
                box.getCenter(),
                new THREE.Quaternion(sbt.x, sbt.y, sbt.z, sbt.w),
                new THREE.Vector3(dim.x, dim.y, dim.z)
            );

            const sceneToSectionBox = (new THREE.Matrix4())
                .copy(transform)
                .multiply(modelToSectionBox);

            planes = SceneMath.box2CutPlanes(
                new THREE.Box3(
...
Viewer3D.prototype.activateLayerState = function(stateName) {
    ...
    this.dispatchEvent({
        type: et.LAYER_VISIBILITY_CHANGED_EVENT
    });
};
 ...
 */
Viewer3D.prototype.search = function(text, onSuccessCallback, onErrorCallback, attributeNames, options = {
    searchHidden: false
}) {
 ...
 * @param {Boolean} [fireEvent=true] - Whether an event is fired at the end
 * @alias Autodesk.Viewing.Viewer3D#setAggregateSelection
 */
Viewer3D.prototype.setAggregateSelection = function(selection, fireEvent = true) {
    this.impl.selector.setAggregateSelection(selection, fireEvent);
};
 ...
 */
Viewer3D.prototype.explode = function(scale, options = {}) {
    if (!this.model)
        return;
    ...
    var exploded = this.impl.explode(scale, options);
    if (exploded) {
        logger.track({
            name: 'explode_count',
            aggregate: 'count'
 ...
 * Not applicable to 2D.
 *
 * @returns {object} `{magnitude:Number, depthDampening:Number}`
 *
 * @alias Autodesk.Viewing.Viewer3D#getExplodeOptions
 */
Viewer3D.prototype.getExplodeOptions = function() {
    if (this.model && this.model.is2d()) {
        logger.warn("Viewer3D.getExplodeOptions is not applicable to 2D");
        return 0;
    }

    return this.impl.getExplodeOptions();
};
...
Viewer3D.prototype.setNavigationLock = function(value) {
    ...
    if (prev !== value) {
        ...
        this.dispatchEvent({
            type: et.NAVIGATION_MODE_CHANGED_EVENT,
...
/**
 * @deprecated
 * Hides all fragments in the current model that are marked as hidden from display.
 * NOTE calling this method can take a while depending on model size.
 * NOTE also that this method may not work in all circumstances, but can cause unwanted side effects, including crashing.
 *
 * @alias Autodesk.Viewing.Viewer3D#hideHiddenObjects
 */
Viewer3D.prototype.hideHiddenObjects = function() {
    let model = this.impl.model;
    this.impl.handleInitialVisibility(model);

    analytics.track('viewer.hideHiddenObjects', {});
};
...
Viewer3D.prototype.initContextMenu = function() {
    ...
    this.onMouseUp = function(event) {
        if (EventUtils.isRightClick(event) && event.clientX === self.startX && event.clientY === self.startY) {
            ...
            // Option provides right click selection of objects
            const isRightBtnEnabled = this.viewer.impl.isRightBtnSelectionEnabled();
            if (isRightBtnEnabled) {
                const vpVec = this.viewer.impl.clientToViewport(event.clientX, event.clientY);
                const result = this.viewer.impl.hitTestViewport(vpVec, false);
                const selectedDbId = result ? .dbId;
                const currentSelection = this.viewer.getSelection();

                // If new dbId selected and it's different from currently selected
                // Don's show contextual menu but select new object only
                if (selectedDbId !== undefined && !currentSelection.includes(selectedDbId)) {
                    return;
                }
                // If no dbIds selected and previously selected any
                // Don't show contextual menu and deselect everything
                if (selectedDbId === undefined && currentSelection.length) {
                    return;
                }
            }
 ...
 * @param {boolean} [ignore2dModelsOn3d] - Whether to ignore 2d models when in 3d mode.
 ...
 */
Viewer3D.prototype.clientToWorld = function(clientX, clientY, ignoreTransparent, ignore2dModelBounds, ignore2dModelsOn3d) {

    return this.impl.clientToWorld(clientX, clientY, ignoreTransparent, ignore2dModelBounds, ignore2dModelsOn3d);
};
 ...
 * @param {Autodesk.Viewing.Model} [model] - For multi-model support.
 * 
 * @alias Autodesk.Viewing.Viewer3D#setMaterialsToDefaults
 */
Viewer3D.prototype.setMaterialsToDefaults = function(model) {
    const fragmentList = (model || this.model).getFragmentList();
    if (!fragmentList) {
        return;
    }

    // Since we use fragment consolidation in some cases, we have to reconsolidate model in order to apply materials.
    const modelConsolidated = model.isConsolidated();
    if (modelConsolidated) {
        model.unconsolidate(false, true);
    }

    // Restores materials from FragmentList data
    fragmentList.restoreOriginalMaterials();

    //Re-consolidate model after applying material overrides.
    //Do not consolidate if model was not uncolidated beforehand.
    if (modelConsolidated && !model.isConsolidated()) {
        this.impl.consolidateModel(model);
    }
    this.impl.invalidate(true, true);
...
/**
 * Returns the first model, according to the environment.
 * If we are in 2D, returns the first sheet.
 * If we are in 3D, returns the first 3D model, regardless if a 2D sheet was loaded before.
 * Note: If there's only 2D models in a 3D environment it will return null.
 * @returns {Autodesk.Viewing.Model} - A model
 *
 * @alias Autodesk.Viewing.Viewer3D#getFirstModel
 */
Viewer3D.prototype.getFirstModel = function() {
    let firstModel = this.impl.is2d ? this.impl.get2DModels()[0] : this.impl.get3DModels()[0];
...
Viewer3D.prototype.restoreDefaultSettings = function() {
    ...
    this.fireEvent({
        type: et.RESTORE_DEFAULT_SETTINGS_EVENT
    });
};
 ...
 * @property {number} fragId - ID of Viewer SDK *fragment* that was intersected.
...
Viewer3D.prototype.refresh = function(clear) {
    this.impl.invalidate(clear, false, !clear, !clear);
};
