import {
    PIXEL_CULLING_THRESHOLD
} from "../wgs/globals.js";
import {
    FrustumIntersector
} from '../wgs/scene/FrustumIntersector';
import {
    RenderContext
...
function getScreenShotLegacy(viewer, w, h, cb, overlayRenderer) {
    ...
    if (viewer.model.isLeaflet()) {
        ...
        viewer.addEventListener(et.CANCEL_LEAFLET_SCREENSHOT, cancelCB, {
            once: true
        });
...
//                                         it is called with (renderContext, renderTarget, camera, sheetRenderer, extraTargets) instead.
...
//  @param {boolean}            [options.renderOverlayScenes] - Render overlay scenes in final step to overlayTarget. If options.overlayScenes is not provided, renders all overlay scenes.
//  @param {Object}             [options.overlayScenes]       - An object containing overlay scenes to render to overlayTarget
//  @param {Array}              [options.extraTargets]   - Extra named targets to return in onFinished callback
...
const getScreenShot = (w, h, onFinished, options, viewerImpl) => {
    ...
    const frustum = new FrustumIntersector();
    const cutplanes = options.excludeCutPlanes ? [] : materials.getCutPlanes();
    const cutplanesHideInterior = options.excludeCutplanes ? false : materials.cutplanesHideInterior > 0;
    frustum.reset(camera, cutplanes, cutplanesHideInterior);
    ...
    const onRenderDone = () => {
        ...
        if (needsPostProcess) {
            ...
            let lights = viewerImpl.lightsOn ? viewerImpl.lights : viewerImpl.no_lights;

            if (options.overlayScenes) {
                const overlayScenes = options.overlayScenes ? ? viewerImpl.overlayScenes;
                ctx.renderOverlays(overlayScenes, lights);
            }
        }

        if (options.returnAsTarget) {
            const extraTargets = options.extraTargets ? options.extraTargets.map(target => ctx.getNamedTarget(target)) : undefined;
            onFinished(ctx, finalTarget, camera, sheetRenderer, extraTargets);
            return;
        }
...
const getCameraWithFitBounds = (viewer, bounds, margin) => {
    ...
    const camState = viewer.getState({
        viewport: true
    });
