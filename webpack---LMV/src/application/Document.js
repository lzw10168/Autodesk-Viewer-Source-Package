...
import {
    endpoint,
    isOffline,
    getOfflineResourcePrefix,
    getEnv,
    setEnv
} from "../file-loaders/net/endpoints";
import {
    ViewingService
 ...
 * by id.
 *
 * Typically, you load the document from APS, parse it for
...
/**
 * Invoked after fetching a JSON manifest from APS.
 * 
 * @callback Autodesk.Viewing.Document~loadSuccessCallback
 * @param {Autodesk.Viewing.Document} doc - Instance that wraps the APS JSON response.
 */

/**
 * Invoked after failing to fetch a JSON manifest from APS.
...
Document.load = function(documentId, onSuccessCallback, onErrorCallback, accessControlProperties, options = {}) {
    const documentPath = Document.getDocumentPath(documentId);
    let acmSession = undefined; // set by doLoad below
    ...
    const apacPatterns = ['urn:adsk.wipprodanz'];
    if ((documentId.startsWith('urn') && urnMatches(apacPatterns)) || endpoint.getApiFlavor().match(/APAC/i)) {

        const api = endpoint.getApiFlavor();
        const env = getEnv();
        if (env.match(/prod/i) !== null) {
            endpoint.setEndpointAndApi("https://cdn-apac.derivative.autodesk.com", api);
        } else if (env.match(/stag/i) !== null) {
            endpoint.setEndpointAndApi("https://cdn-apac-stg.derivative.autodesk.com", api);
        }
    }
    ...
    function onSuccess(data) {
        ...
        if (endpoint.isSVF2Backend() || endpoint.isOtgBackend()) {
            // Fluent endpoint uses the original manifest URN as acm session, 
            // so we expand the acmSession query parameter with the added fluent server piece.
            // This is because some manifests are "shallow copies" of other manifests, 
            // and the URN is used to authorize access to the storage context of the source data. 
            // Setting an acmsession for Fluent data is only needed in case of shallow copy urns.
            if (acmSession) {
        ...
        const lmvDocument = new Document(data, documentPath, acmSession);

        messages = lmvDocument.getGlobalMessages();

        // If there are no viewables, report an error if possible
        const containsViewables = lmvDocument.docRoot.findAllViewables().length > 0;
        if (!containsViewables) {
            const errorCode = ErrorCodes.BAD_DATA_NO_VIEWABLE_CONTENT;
            const errorMsg = "No viewable content";
            onErrorCallback && onErrorCallback(errorCode, errorMsg, messages);
            return;
        }

        onSuccessCallback && onSuccessCallback(lmvDocument, messages);
    }
    ...
    function doLoad() {
        ...
        const canSkipAcmSession = !accessControlProperties || ((endpoint.isSVF2Backend() || endpoint.isOtgBackend()) && !accessControlProperties["x-ads-acm-scopes"]);
        ...
        const loadContext = endpoint.initLoadContext(options);
        ViewingService.getManifest(loadContext, documentPath, onSuccess, onFailure);
    }
...
Document.requestThumbnailWithSecurity = function(data, onComplete) {
    ...
    ViewingService.getThumbnail(endpoint.initLoadContext({
        endpoint: endpointUrl
    }), urlpath, onSuccess, onFailure, options);
};
...
Document.prototype.getFullPath = function(urn) {
    ...
    if (isOffline()) {
        ...
        if (!isFluentUrn && (endpoint.isSVF2Backend() || endpoint.isOtgBackend())) {
            ...
            //stick to option (1), which is enabled by USE_OTG_DS_PROXY. For Docs and APS we use the redirection.
...
Document.prototype.getLeafletZipParams = function(outLoadOptions, geomItem) {
    var leafletZipItem = geomItem.search({
        'role': 'leaflet-zip'
    }, false);
...
Document.prototype.downloadAecModelData = function(onFinished) {
    ...
    // 'absPath' is against APS-API / derivative service we need to provide the authorization header.
    // NOTE: the normal `endpoint.HTTP_REQUEST_HEADERS["Authorization"]` mechanism (see envinit.js) doesn't work here
    // since we basically use a different endpoint (APS API instead of OTG service).
