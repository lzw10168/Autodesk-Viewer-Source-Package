...
export function ExtensionEventsMixin(extension) {
    ...
    intercept('load', function() {
        this.viewer.dispatchEvent({
            type: av.EXTENSION_PRE_LOADED_EVENT,
            extensionId: this.id
        });
    });
    ...
    intercept('unload', function() {
        this.viewer.dispatchEvent({
            type: av.EXTENSION_PRE_UNLOADED_EVENT,
            extensionId: this.id
        });
    });
    ...
    intercept(
        ...
        function(mode) {
            this.viewer.dispatchEvent({
                type: av.EXTENSION_PRE_ACTIVATED_EVENT,
                extensionId: this.id,
                mode: mode
            });
        },
        function(mode) {
            this.viewer.dispatchEvent({
                type: av.EXTENSION_ACTIVATED_EVENT,
                extensionId: this.id,
                mode: mode
            });
            analytics.track('viewer.extension.activate', {
                extensionId: this.id,
                mode
            });
        }
    ...
    intercept(
        ...
        function() {
            this.viewer.dispatchEvent({
                type: av.EXTENSION_PRE_DEACTIVATED_EVENT,
                extensionId: this.id
            });
        },
        function() {
            this.viewer.dispatchEvent({
                type: av.EXTENSION_DEACTIVATED_EVENT,
                extensionId: this.id
            });
            analytics.track('viewer.extension.deactivate', {
                extensionId: this.id
            });
        }
    );
