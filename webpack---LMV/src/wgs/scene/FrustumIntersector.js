...
FrustumIntersector.prototype.reset = function(camera, cutPlanes, cutplanesHideInterior = false) {
    ...
    if (this.perspective) {
        this.projScale = camera.clientHeight / (2.0 * Math.tan(THREE.Math.degToRad(camera.fov / 2)));
    } else {
        this.projScale = camera.clientHeight / camera.orthoScale;
    }
};
...
FrustumIntersector.prototype.projectedBoxArea = (function() {
    ...
    function clip(clipPolygon, sizePolygon) {
        ...
        for (numClip = 0;
            (numClip < 4) && (sizePolygon > 2); numClip++) {
            newSizePolygon = 0;
...
// This function underestimates the projected diameter for increased speed. Use it with care. As a rule of thumb, only use it for bboxes inside the frustum.
// - it uses the max length along an axis instead of the actual box diameter. For a cube, this means an error of sqrt(3), 
//   but frequently a bounding sphere of the geometry is smaller than the bounding sphere of the bounding box.
// - for perspective cameras the formula is derived for large distances at the center of the screen. 
//   At screen boundaries with large fov it will be of by a factor of 1.5 or more.
// - for perspective cameras, the result is limited to avoid infinities/NaNs or negative values
FrustumIntersector.prototype.estimateProjectedDiameter = function(bbox) {
    ...
    // rough diameter of the box
    const d = Math.max(bbox.max.x - bbox.min.x, bbox.max.y - bbox.min.y, bbox.max.z - bbox.min.z);

    if (this.perspective) {
        // I'd love to write this: const depth = dot(bboxCenter - this.eye, this.viewDir);
        // I have to write this:
        const depth = (x - this.eye.x) * this.viewDir[0] + (y - this.eye.y) * this.viewDir[1] + (z - this.eye.z) * this.viewDir[2]; // assumes viewDir to be normalized
        return d < depth ? d * this.projScale / depth : this.projScale;
    } else {
        return d * this.projScale;
    }
};
...
FrustumIntersector.prototype.boxOutsideCutPlanes = function(box) {
    if (this.cutplanesHideInterior || !this.cutPlanes) {
        return boxInsideConvex(box, this.cutPlanes);
    }
...
function boxInsideConvex(box, cutplanes = []) {
    if (!_tmpPlane) _tmpPlane = new THREE.Vector4();
    let insideCount = 0;
    for (let i = 0; i < cutplanes.length; i++) {
        _tmpPlane.copy(cutplanes[i]).negate();
        if (boxOutsideCutPlane(box, _tmpPlane)) {
            insideCount++;
        }
    }
    return cutplanes.length > 3 && insideCount === cutplanes.length;
}