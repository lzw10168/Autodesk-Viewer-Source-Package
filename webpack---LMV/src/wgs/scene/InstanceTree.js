 ...
 * @param {Autodesk.Viewing.Private.InstanceTree~onEnumNodeFragments} callback - The function that will be called for each fragment.
 * Note that if the callback function returns a truthy value, a loop over the fragments and child nodes will be interrupted
 * and the callback result will be forwarded back to the caller.
 ...
 * @param {number} dbId - The id of a child node or, if `parentId` is not provided,
 * the id of the initial node
 * @param {number|undefined} parentId - The id of a parent node. If not provided, the `dbId` refers
 * to the initial node. 
 */
 ...
 * 
 * @param {number} node - The id of a node.
 * @param {Autodesk.Viewing.Private.InstanceTree~onEnumNodeChildren} callback - The function that will be called for each child node.
 * Note that if the callback function returns a truthy value, a loop over the child nodes will be interrupted
 * and the callback result will be forwarded back to the caller.
...
InstanceTree.prototype.enumNodeChildren = function(node, callback, recursive) {
    ...
    const traverse = (dbId) => {
        var res = this.nodeAccess.enumNodeChildren(dbId, (childId) => {
            // TODO: make this logic more explicit for consumers of this function
            // or (ideally) get rid of it i.e. a refactoring required.
            // Don't forget to refactor `enumNodeFragments` as well.
            if (callback(childId, dbId))
                return childId;
