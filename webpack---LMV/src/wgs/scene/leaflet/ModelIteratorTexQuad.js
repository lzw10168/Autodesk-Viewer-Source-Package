...
} from "../../render/ShaderUtils";
import {
    LeafletShader
} from "../../render/LeafletShader";
...
export function ModelIteratorTexQuad(config, materials) {
    ...
    /** @returns {bool} */
    this.done = function(viewIndex) {
        return _done[viewIndex || 0];
    };
    ...
    var resizeToPow2 = function(tex) {
        ...
        var w = 1;
        while ((w *= 2) < image.width);
        var h = 1;
        while ((h *= 2) < image.height);
    ...
    this.reset = function(frustum, camera, viewIndex) {
        ...
        ({
            camera,
            frustum
        } = applyTransformIfNeeded(scene, frustum, camera));
