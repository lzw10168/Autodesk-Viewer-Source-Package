...
function createMergeTask(geoms, targetGeom, matrices, dbIds) {
    ...
    task.posOffset = targetGeom.attributes.position.offset;
    task.normalOffset = (targetGeom.attributes.normal ? targetGeom.attributes.normal.offset : -1); // -1 for "no normals"
...
export function ParallelGeomMerge(consolidation) {
    ...
    this.runTasks = function() {
        ...
        for (var i = 0; i < numWorkers; i++) {
            ...
            _workers[i].addEventListener('message', handleGeomMergeResult);
        }
