...
function convertTexture(textureDef, texture, sceneUnit, maxAnisotropy) {
    ...
    // transform params if defined
    texture.offset = textureDef.offset ? ? texture.offset;
    texture.rotation = textureDef.rotation ? ? texture.rotation ? ? 0;
    texture.repeat = textureDef.repeat ? ? texture.repeat;
...
function applyTextureTransformExtension(texture, map) {
    map.offset = new THREE.Vector2(0, 0);
    ...
    if (transform && typeof transform === 'object') {
        ...
        if (offset) map.offset = new THREE.Vector2(offset[0], offset[1]);
        if (scale) map.repeat = new THREE.Vector2(scale[0], scale[1]);
        if (rotation) map.rotation = rotation;
    }
}
...
function convertMaterialGltf(matObj, svf) {
    ...
    if (diffuse) {
        if (Array.isArray(diffuse)) {
            ...
            tm.textureMaps[map.mapName] = map;
            applyTextureTransformExtension(diffuse, map);
        }
    }
    ...
    if (metallicRoughnessTexture) {
        ...
        map.flipY = false;
        tm.textureMaps[map.mapName] = map;
        applyTextureTransformExtension(metallicRoughnessTexture, map);
    } else {
    ...
    if (matObj.normalTexture !== undefined) {
        ...
        map.flipY = false;
        map.uri = svf.gltf.images[texture.source].uri;
        applyTextureTransformExtension(matObj.normalTexture, map);
        // `repeat` is a Three.js notation for `scale`, e.g., see https://github.com/mrdoob/three.js/pull/11863
        tm.normalScale = map.repeat;
    ...
    if (matObj.occlusionTexture !== undefined) {
        ...
        applyTextureTransformExtension(matObj.occlusionTexture, map);
    }
    ...
    if (matObj.emissiveTexture !== undefined) {
        ...
        map.flipY = false;
        tm.textureMaps[map.mapName] = map;
        applyTextureTransformExtension(matObj.emissiveTexture, map);
    }
    return tm;
}
