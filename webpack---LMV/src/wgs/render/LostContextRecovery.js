...
export default class LostContextRecovery {
    ...
    _refreshBufferGeometryGlData(geom) {
        ...
        if (geom.__webglInit) {
            geom.__webglInit = undefined;
            geom.needsUpdate = true;
        }
        if (geom.vbbuffer) {
            geom.vbbuffer = undefined;
            geom.needsUpdate = true;
        }
        if (geom.ibbuffer) {
            geom.ibbuffer = undefined;
            geom.needsUpdate = true;
        }
        if (geom.iblinesbuffer) {
            geom.iblinesbuffer = undefined;
            geom.needsUpdate = true;
        }
        if (geom.vaos) {
            geom.vaos = undefined;
    ...
    _refreshTextureGlData(tex) {
        ...
        if (tex.__webglTextureCube) {
            tex.__webglTextureCube = null;
            tex.needsUpdate = true;
        }
    }
