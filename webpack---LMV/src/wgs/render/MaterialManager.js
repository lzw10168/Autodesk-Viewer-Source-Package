...
export function MaterialManager(renderer) {
    ...
    this._layerMaskTextures = {}; // same as for selection textures: "yes, an object and not an array."
    this._layerMaps = {};
...
MaterialManager.prototype._getTextureHash = function(model, imageUri, mapName, materialId) {
    ...
    // Upd. Also added material ID to allow same texture having different configuration per material.
    // This shouldn't impact the performance as "real" texture caching happens already inside three.js.
    return `${this._getModelHash(model)}mat:${materialId}|tex:${imageUri}|map:${mapName}`;
};
...
MaterialManager.prototype._applyCutPlanes = function(mat) {
    ...
    mat.cutplanes = this._cutplanes;
    mat.cutplanesHideInterior = this.cutplanesHideInterior;
...
MaterialManager.prototype.setTextureInCache = function(model, material, map, tex) {

    // Texture loaded successfully
    var texName = this._getTextureHash(model, map.uri, map.mapName, material.id);
    ...
    if (mat.textureMaps) {
        for (var mapName in mat.textureMaps) {
            if (mat[mapName]) {
                ...
                var texName = this._getTextureHash(model, texUri, sharedMapName, mat.id);
    ...
    if (threegeom.isLines || threegeom.isWideLines || threegeom.isPoints) {
        ...
        if (threegeom.isPoints) {
            ...
            if (threegeom.isWideLines) {
                if (!material) {
                    material = svfmat[cache] = new THREE.MeshBasicMaterial({
                        vertexColors: vertexColors
                    });
                ...
                material = svfmat[cache] = new THREE.LineBasicMaterial({
                    vertexColors: vertexColors
                });
            }
        }
...
MaterialManager.prototype.cleanup = function(model) {
    ...
    // Dispose layer mask texture
    var disposeLayerMaskTex = (modelId, removeKey) => {
        const layerTex = this._layerMaskTextures[modelId];
        if (layerTex === undefined) {
            return;
        }
        ...
        if (removeKey) {
            delete this._layerMaskTextures[modelId];
        }
    };
    ...
    if (model) {
        ...
        disposeLayerMaskTex(model.id, true);
        delete this._layerMaps[model.id]; // in contrast to selection and layer masks, layer maps are not owned by the manager
    } else {
        ...
        // dispose all layerMask textures
        for (var key in this._layerMaskTextures) {
            if (this._layerMaskTextures.hasOwnProperty(key)) {
                disposeLayerMaskTex(key, false);
            }
        }
 ...
 */
MaterialManager.prototype.setCutPlanes = function(cutplanes, cutplanesHideInterior = 0) {
    // Update shaders if num of planes changed
    var doubleSided = false;

    const cutplaneExistenceChanged = this._cutplanes.length !== (cutplanes ? cutplanes.length || 0 : 0),
        cutplanesHideInteriorToggled = cutplanesHideInterior !== this.cutplanesHideInterior;

    // Check if we need to recompile material shaders. This happens (only) if the cutplane count
    // changes between 0 and !=0 or the cutplanes get inverted as a convex shape
    // TODO: why do we need to recompile the shaders when the convex flag is toggled, it is only a uniform that can be changed...
    //    Currently does not work without recompiling when toggled
    const needsUpdate = cutplaneExistenceChanged || cutplanesHideInteriorToggled;

    // Empty array (http://jsperf.com/empty-javascript-array)
    while (this._cutplanes.length > 0) this._cutplanes.pop();
    this.cutplanesHideInterior = cutplanesHideInterior;
...
MaterialManager.prototype._applyMRTFlags = function(mat) {
    ...
    if (!mat.skipMrtNormals) {
        mat.mrtNormals = matSupportsMrtNormals && this._mrtNormals;
    }

    mat.mrtIdBuffer = this._mrtIdBuffer;
...
MaterialManager.prototype.create2DMaterial = function(model, material, isIdMaterial, selectionTexture, onReady) {
    ...
    if (!this._materials.hasOwnProperty(hash)) {
        ...
        if (!material.skipMiterLines) {
            lineMaterial.defines["HAS_MITER_LINES"] = 1;
        }
