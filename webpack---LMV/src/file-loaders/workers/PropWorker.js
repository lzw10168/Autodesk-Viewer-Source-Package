...
import {
    PropertyDatabaseCustomPropertyWrapper,
    acquireCustomPropsCache
} from '../lmvtk/common/PropdbCustomPropertyWrapper.js';
...
function loadPropertyPacks(loadContext, dbId, onPropertyPackLoadComplete) {
    ...
    if (dbfiles.avs.length) {
        ...
        for (var tag in dbfiles) {
            ...
            loadedDbFiles[tag] = {};
            filesToRequest.push({
                filename: dbfiles[tag][0].path,
                isShared: dbfiles[tag][0].isShared,
                storage: loadedDbFiles[tag]
            });
        }
    ...
    function onRequestCompletion(data) {
        ...
        if (!filesRemaining) {
            if (filesFailed) {
                ...
                try {
                    cacheEntry.pdb = loadContext.useCustomProperties ? new PropertyDatabaseCustomPropertyWrapper(loadedDbFiles) : new PropertyDatabase(loadedDbFiles);
...
function doPropertyGet(loadContext) {
    ...
    if (!results) {
        loadContext.worker.postMessage({
            cbId: loadContext.cbId,
            error: {
                msg: "Properties are not available."
            }
        });
    } else {
        loadContext.worker.postMessage({
...
function doLoadExternalIds(loadContext) {
    ...
    var idsFileEntry = loadContext.idsFile || {
        path: "objects_ids.json.gz"
    };
...
function doExecuteCode(loadContext) {
    ...
    if (loadContext.userFunction) {
        try {
            ...
            _this.postMessage({
                cbId: loadContext.cbId,
                error: {
                    msg: "Error while importing 'userFunction'."
                }
            });
            return;
        }
    ...
    if (!self.userFunction) {
        _this.postMessage({
            cbId: loadContext.cbId,
            error: {
                msg: "function 'userFunction' was not found."
            }
        });
        return;
    }
    ...
    if (cacheEntry && cacheEntry.pdb) {
        ...
        try {
            ...
            _this.postMessage({
                cbId: loadContext.cbId,
                error: {
                    msg: "Error while executing 'userFunction'."
                }
            });
            return;
        } finally {
...
async function doRefreshCustomProperties(loadContext) {
    ...
    const customPropsCache = acquireCustomPropsCache(_this);
    let result, error;
    try {
...
async function doRefreshCustomPropertiesValues(loadContext) {
    ...
    try {
        ...
        var pdb = _this.pdbCache ? .get(dbPath) ? .pdb;
...
export function register(workerMain) {
    workerMain.register("BUILD_EXTERNAL_ID_MAPPING", {
        doOperation: doBuildExternalIdMapping
    });
    workerMain.register("BUILD_LAYER_TO_NODE_ID_MAPPING", {
        doOperation: doBuildLayerToNodeIdMapping
    });
    workerMain.register("GET_PROPERTIES", {
        doOperation: doPropertyGet
    });
    workerMain.register("GET_PROPERTY_SET", {
        doOperation: doPropertySetGet
    });
    workerMain.register("SEARCH_PROPERTIES", {
        doOperation: doPropertySearch
    });
    workerMain.register("DIFF_PROPERTIES", {
        doOperation: doPropertyDiff
    });
    workerMain.register("FIND_PROPERTY", {
        doOperation: doPropertyFind
    });
    workerMain.register("FIND_LAYERS", {
        doOperation: doLayersFind
    });
    workerMain.register("LOAD_PROPERTYDB", {
        doOperation: doObjectTreeParse
    });
    workerMain.register("CREATE_TREE", {
        doOperation: doObjectTreeCreate
    });
    workerMain.register("UNLOAD_PROPERTYDB", {
        doOperation: doUnloadPropertyDb
    });
    workerMain.register("USER_FUNCTION", {
        doOperation: doExecuteCode
    });
    workerMain.register("LOAD_EXTERNAL_IDS", {
        doOperation: doLoadExternalIds
    });
    workerMain.register("REFRESH_CUSTOM_PROPERTIES", {
        doOperation: doRefreshCustomProperties
    });
    workerMain.register("REFRESH_CUSTOM_PROPERTIES_VALUES", {
        doOperation: doRefreshCustomPropertiesValues
    });
}