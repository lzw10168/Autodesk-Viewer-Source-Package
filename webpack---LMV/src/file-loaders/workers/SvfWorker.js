import {
    Package
} from '../lmvtk/svf/Package';
import {
    InputStream
} from '../lmvtk/common/InputStream';
import {
    ViewingService
} from '../net/Xhr';
import {
...
function doLoadSvf(loadContext) {
    ...
    _this.postMessage({
        progress: 0.01
    }); //Tell the main thread we are alive
...
export function register(workerMain) {
    workerMain.register("LOAD_SVF", {
        doOperation: doLoadSvf
    });
    workerMain.register("LOAD_SVF_CONTD", {
        doOperation: doLoadSvfContinued
    });
    workerMain.register("FETCH_TOPOLOGY", {
        doOperation: doFetchTopology
    });
}