...
function getSingleFileFromZip(zip, filename) {
    const unzipped = unzipSync(zip, {
        filter: fileinfo => fileinfo.name === filename
    })[filename];
    if (!unzipped) {
...
Package.prototype.loadManifest = function(loadContext) {
    ...
    const jdr = new InputStream(manifestJson);
    ...
    for (let i = 0; i < assets.length; i++) {
        ...
        if (uri.indexOf("embed:/") != 0) {
            if (type == "PackFile") {
                ...
                this.propertydb.attrs.push({
                    path: uri
                });
            } else if (type == "PropertyAVs") {
                this.propertydb.avs.push({
                    path: uri
                });
            } else if (type == "PropertyIDs") {
                this.propertydb.ids.push({
                    path: uri
                });
            } else if (type == "PropertyOffsets") {
                this.propertydb.offsets.push({
                    path: uri
                });
            } else if (type == "PropertyValues") {
                this.propertydb.values.push({
                    path: uri
                });
            }
        }
        ...
        if (type == "ProteinMaterials") {
            ...
            } else if (path.indexOf("OverrideSets.bin") != -1) {
                this.loadAsyncResource(loadContext, path, contents, function(data) {
                    svf.overrideSetsPack = new PackFileReader(data);
                });
            } else if (path.indexOf("Viewpoints.bin") != -1) {
                this.loadAsyncResource(loadContext, path, contents, function(data) {
                    svf.viewpointDefPack = new PackFileReader(data);
                });
            } else if (path.indexOf("ViewpointTree.bin") != -1) {
                this.loadAsyncResource(loadContext, path, contents, function(data) {
...
Package.prototype.postLoadOfOverrideSets = function(loadContext) {
    if (this.overrideSetsPack) {
        for (let i = 0, iEnd = this.overrideSetsPack.getEntryCounts(); i < iEnd; i++) {
            const set = readOverrideSet(this.overrideSetsPack, i);
...
Package.prototype.postLoadOfViewpointTree = function(loadContext) {
    if (this.viewpointTreePack) {
        const root = readNamedItemTree(this.viewpointTreePack);
...
Package.prototype.postLoadOfViewpoints = function(loadContext) {
    if (this.viewpointDefPack && this.camDefPack) {
        for (let i = 0, iEnd = this.viewpointDefPack.getEntryCounts(); i < iEnd; i++) {
            const def = readViewpointDefinition(this.viewpointDefPack, this.camDefPack, i);
