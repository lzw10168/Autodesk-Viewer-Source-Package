...
import {
    allocateUintArray
} from "../../../wgs/scene/IntArray";
import {
    ProgressiveReadContext
} from "./ProgressiveReadContext";
import {
    getGlobal
} from "../../../global";
import {
    PackFileReader
} from '../common/PackReader';
import {
    readViewpointDefinition
} from '../common/Viewpoints';
import {
    readOverrideSet
} from '../common/OverrideSets';
import {
    readNamedItemTree
} from '../common/NamedItemTree';
...
OtgPackage.prototype.getMaterialHash = function(materialIndex) {
    ...
    var matHash = binToPackedString(this.materialHashes.hashes, materialIndex * stride, stride);
 ...
 */
OtgPackage.prototype.getExtraMaterialHashes = function() {
    ...
    const extraMatIds = [...matIndexSet].filter(x => !this.materialIdToHash[x]);

    if (extraMatIds.length) {
        this.needExtraMaterials = true;
    }

    // get the hash string that points to the material
    for (const extraMaterialIndex of extraMatIds) {
        var matHash = this.getMaterialHash(extraMaterialIndex);
        this.extraMatHashes[extraMaterialIndex] = matHash;
    }
};
...
OtgPackage.prototype.getGeometryHash = function(geomIndex) {
    ...
    var gHash = binToPackedString(this.geomMetadata.hashes, geomIndex * stride, stride);
...
OtgPackage.prototype.loadAsyncProgressive = function(loadContext, resourcePath, ctx, resourceName, onFailureCallback) {
    ...
    const useFetch = getGlobal().USE_FETCH_API; // Use fetch instead of XMLHttpRequest for progressive loading.
...
OtgPackage.prototype.postLoadViewpointData = function(loadContext) {
    this.viewpointProgress++;

    // All the packfiles are ready
    if (this.viewpointProgress === 4) {
        this.processViewpoints();
        this.getExtraMaterialHashes();
