...
import {
    AttributeType,
    AttributeFlags,
    RVT_DIM_PROPS_BLACKLISTED,
    IFC_NWD_ITEM_PROPS_BLACKLISTED
} from './PropdbEnums';
...
export function PropertyDatabase(dbjsons) {
    ...
    this.getNodeNameAndChildren = function(node /* {dbId:X, name:""} */ , skipChildren) {
        ...
        if ((!node.name || !node.flags) && instanceOfValId) {
            ...
            if (!tmp) {
                ...
                _instanceNodeCache[dbIdOfInstance] = tmp = {
                    dbId: dbIdOfInstance,
                    name: null,
                    flags: null
                };
    ...
    this._attributeIsBlacklisted = function(attrId) {
        ...
        if (Object.prototype.hasOwnProperty.call(IFC_NWD_ITEM_PROPS_BLACKLISTED, category) && IFC_NWD_ITEM_PROPS_BLACKLISTED[category].indexOf(name) !== -1)
            return true;
        ...
        if (Object.prototype.hasOwnProperty.call(RVT_DIM_PROPS_BLACKLISTED, category) && RVT_DIM_PROPS_BLACKLISTED[category].indexOf(name) !== -1)
            return true;
    ...
    this.findDifferences = function(dbToCompare, diffOptions, onProgress) {
        ...
        var cachedParentAttribs1 = {
            result: [],
            dbId: -1
        };
        var cachedParentAttribs2 = {
            result: [],
            dbId: -1
        };
    ...
    this.numberOfValues = function() {
        return _valuesOffsets ? .length ? ? 0;
    };
