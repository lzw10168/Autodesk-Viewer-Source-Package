import {
    VertexBufferBuilder
} from "../common/VertexBufferBuilder";
import {
    LmvBox3
    ...
    if (manifest) {
        ...
        for (var i = 0, e = assets.length; i < e; ++i) {
            ...
            if (entry.type === "Autodesk.CloudPlatform.PropertyViewables")
                this.propertydb.viewables.push({
                    path: entry.URI
                });
    ...
    if (this.metadata) {
        ...
        if (!this.noShadow) {
            ...
            var ssw = {
                x: dw.x * ss / pw,
                y: dw.y * ss / pw
            }; // shadow offset in width direction
            var ssh = {
                x: dh.x * ss / ph,
                y: dh.y * ss / ph
            }; // shadow offset in height direction
            var ssb = {
                x: o.x + ssw.x,
                y: o.y + ssw.y
            }; // bottom shadow origin
            var ssr = {
                x: o.x + dw.x,
                y: o.y + dw.y
            }; // right shadow origin
...
F2D.prototype.parseString = function() {
    ...
    switch (sema) {
        case F2dSemanticType.st_object_member:
            ...
            break;
        default:
            logger.info("Unexpected opcode semantic type for string.");
            break;
    }
...
F2D.prototype.parsePoint = function() {
    ...
    switch (sema) {
        case F2dSemanticType.st_object_member:
            ...
            break;
        default:
            logger.info("Unexpected opcode semantic type for point.");
            break;
    }
...
F2D.prototype.parseCircularArc = function() {
    ...
    var major = s.getVarints(),
        /*rotation = s.getFloat32(),*/ start = s.getFloat32(),
        end = s.getFloat32();
...
F2D.prototype.createLayerGroups = function(layers) {
    ...
    function sortLayers(parent) {
        var children = Object.keys(parent.childrenByName).map(function(k) {
            return parent.childrenByName[k];
        });
        ...
        if (children.length) {
            ...
            children.sort(function(a, b) {
                ...
                return a.name.localeCompare(b.name, undefined, {
                    sensitivity: 'base',
                    numeric: true
                }); // Sort layers and groups by name
            });
        }
