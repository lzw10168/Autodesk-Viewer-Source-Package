...
import {
    ModelIteratorTexQuad
} from "../../wgs/scene/leaflet/ModelIteratorTexQuad";
import {
    TexQuadConfig
} from "../../wgs/scene/leaflet/ModelIteratorTexQuad";
import {
...
export function LeafletLoader(parent) {
    ...
    this.continueLoadFile = function(path, options, onDone, onWorkerStart, config, textureLoader) {
        ...
        let texQuadTraversalController = null;
        ...
        function onLoad() {
            ...
            // Create RenderModel with texQuad scene traversal
            const model = new Model(modelData);
            model.addTraversalController("Custom", texQuadTraversalController);
            model.loader = self;

            _parent.api.dispatchEvent({
            ...
            // Track loading time
            texQuadTraversalController.callWhenRefined(function() {
                var t1 = Date.now();
        ...
        // create iterator 
        texQuadTraversalController = new ModelIteratorTexQuad(config, _parent.getMaterials());

        // Root tile is always needed
        texQuadTraversalController.requestRootTile();
    };
        ...
        if (options.loadOptions && options.loadOptions.loadFromZip) {
            ...
            var areAllZipsParsed = function() {
                return options.loadOptions.zips.every(function(zip) {
                    return Object.keys(zip.fileTable).length > 0;
                });
            };
