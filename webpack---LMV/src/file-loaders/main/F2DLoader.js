...
F2DLoader.prototype.processReceivedMesh = function(mdata) {
    ...
    const geometry = BufferGeometryUtils.meshToGeometry(mdata);

    geometry.unpackXform = mesh.unpackXform;
    ...
    if (mesh.texData) {
        ...
        geometry.tIdColor = tIdColor;
        geometry.vIdColorTexSize = new THREE.Vector2(mesh.texData.length, 1);
    }
    ...
    rm.getGeometryList().addGeometry(geometry, numInstances, mdata.meshIndex + 1);
...
F2DLoader.prototype.onModelRootLoadDone = function(svf) {
    ...
    var t1 = Date.now();
    logger.log("F2D load: " + (t1 - this.t0));
