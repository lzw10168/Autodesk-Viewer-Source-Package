...
import {
    getPolygonCount
} from "../../wgs/scene/BufferGeometryUtils";
import {
    MaterialConverter
...
// Create a worker.
SvfLoader.prototype.createWorker = createWorker;
...
SvfLoader.prototype.loadSvfCB = function(path, options, onDone, onWorkerStart) {
    ...
    var w = this.svfWorker = this.createWorker();
    ...
    var onSVFLoad = async function(ew) {
        ...
        if (ew.data && ew.data.manifest) {
            ...
            if (ew.data.bvh) {
                ...
                scope.viewer3DImpl.api.dispatchEvent({
                    type: et.LOADER_REPAINT_REQUEST_EVENT,
                    loader: scope,
                    model: scope.model
                });
            }
    ...
    w.addEventListener('message', onSVFLoad);
...
SvfLoader.prototype.loadGeometryPack = function(packId, path) {
    ...
    var onMeshLoad = function(ew) {
        if (ew.data && ew.data.meshes) {
            ...
            scope.failedToLoadSomeGeometryPacks = {
                code: ew.data.error.code,
                msg: ew.data.error.msg
            };
        } else {
            //Download failed.
    ...
    if (pw.length < (av.NUM_WORKER_THREADS || NUM_WORKER_THREADS)) {
        ...
        if (allBusy) {
            var wr = this.createWorker();
            wr.addEventListener('message', onMeshLoad);
            wr.queued = 0;
            pw.push(wr);
        }
    }
    ...
    var xfer = {
        ...
        /* mesh IDs treat the pack file id as integer to save on storage in the per-fragment arrays */
        "workerId": workerId,
...
SvfLoader.prototype.processReceivedMesh = function(mdata) {
    ...
    // Reuse the previous index of this geometry, if available.
    const geometryIndex = rm.getFragmentList().getGeometryId(fragIndexes[0]);

    const geometry = meshToGeometry(mdata);

    if (mdata.packId !== undefined) {
        geometry.packId = mdata.packId; // SvfPagingProxy is the only place that needs this
    }
    if (mdata.meshIndex !== undefined) {
        geometry.meshIndex = mdata.meshIndex; // SvfPagingProxy is the only place that needs this
    }

    const geomId = rm.getGeometryList().addGeometry(geometry, fragIndexes.length, geometryIndex);

    const polyCount = getPolygonCount(geometry);
    ...
    if (this.options.onMeshReceived) {
        this.options.onMeshReceived(this.model, geomId, fragIndexes /*, geometry*/ );
    }
...
SvfLoader.prototype.setupCamera = function(camera) {
    ...
    if (!isFinite(camera.position.x + camera.position.y + camera.position.z +
        ...
        camera.target = this.svf.bbox.getCenter(new THREE.Vector3());
        camera.position.copy(camera.target);
        camera.position.z += this.svf.bbox.max.z - this.svf.bbox.min.z;
        camera.up.set(0, 1, 0);
    }
...
SvfLoader.prototype.onModelRootLoadDone = async function(svf) {
    ...
    if (svf.cameras) {
        for (var i = 0; i < svf.cameras.length; i++) {
            this.setupCamera(svf.cameras[i]);
        }
    }
...
SvfLoader.prototype.makeBVH = function(svf) {
    ...
    svf.bvh.build(this.options.bvhOptions || {
        isWeakDevice: isMobileDevice()
    });
...
SvfLoader.prototype.fetchTopologyFile = function(fullpath, onComplete) {
    ...
    var ww = this.createWorker();
    ww.addEventListener('message', onTopology);
