...
import {
    initWorkerScript
} from "./file-loaders/main/WorkerCreator";
import {
    ViewingService
...
var derivativeStreamingAPACUrls = {
    /** SVF2 */
    stg: "https://cdn-apac-stg.derivative.autodesk.com",
    prod: "https://cdn-apac.derivative.autodesk.com",
};

var DerivativeApiData = {
    v2: "derivativeV2",
    v2_eu: "derivativeV2_EU",
    v2_apac: "derivativeV2_APAC",
...
export let EnvironmentConfigurations = Object.freeze({
    ...
    D3SLocalAPAC: {
        ...
        LMV: currentHost,
        UPSTREAM: currentHost,
        UPSTREAM_API_DATA: DerivativeApiData["v2_apac"]
    },
    Test: {
    ...
    FedrampProduction2: {
        ...
        LMV: LmvEndpoints["prod"],
        UPSTREAM: DevApiFedrampUrls["prod"],
        UPSTREAM_API_DATA: DerivativeApiData["v2_fedramp"]
    }
});
...
export function getUpstreamApiData(env, api) {
    if (env.endsWith('EU') || env.endsWith('APAC')) {
...
export function initializeResourceRoot(options) {
    // If the root was specified, do not attempt to derive it.
    if (options.lmvResourceRoot) {
        global.LMV_RESOURCE_ROOT = options.lmvResourceRoot;
        return;
    }
...
export function initializeAuth(onSuccessCallback, options) {
    ...
    function onGetAccessToken(token, expire = 3599) {
        ...
        _token.tokenRefreshInterval = setTimeout(function() {
 ...
 * @param {string} [options.lmvResourceRoot] - The root folder that contains LMV resources, will be derived if not
 * specified.
...
export function Initializer(options, callback) {
    ...
    // It's undocumented on purpose to keep it internally for our tests.
    // TODO: resolve this weirdness of having both async and callback ways in the same function.
    return Promise.all([apiProm, authProm]).then(callback);
}
