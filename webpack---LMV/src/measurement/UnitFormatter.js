...
export function formatValueWithUnits(value, units, type, precision, options) {
    ...
    options = options || {};
    const feetAndInchesSymbols = options.preferLetters ? {
        feet: ' ft',
        inches: ' in'
    } : {
        feet: '\'',
...
export function convertUnits(fromUnits, toUnits, calibrationFactor, d, type, dpi) {
    ...
    switch (toUnits) {
        case M.MILLIMETER:
            toFactor = 1 / U[M.MILLIMETER];
            break;
        case M.CENTIMETER:
            toFactor = 1 / U[M.CENTIMETER];
            break;
        case M.METER:
            toFactor = 1;
            break;
    ...
    switch (fromUnits) {
        case M.MILLIMETER:
            fromFactor = U[M.MILLIMETER];
            break;
        case M.CENTIMETER:
            fromFactor = U[M.CENTIMETER];
            break;
        case M.METER:
            fromFactor = U[M.METER];
            break;
        case M.INCH:
            fromFactor = U[M.INCH];
            break;
