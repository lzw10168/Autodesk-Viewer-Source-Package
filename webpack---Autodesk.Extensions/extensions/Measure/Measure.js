...
MeasureExtension.prototype.load = async function() {
            ...
            switch (prop) {
                case 'units':
                    viewer.dispatchEvent({
                        type: MeasureCommon.Events.DISPLAY_UNITS_CHANGED,
                        units: value
                    });
                    break;
                case 'precision':
                    viewer.dispatchEvent({
                        type: MeasureCommon.Events.PRECISION_CHANGED,
                        precision: value
                    });
                    break;
    ...
    if (viewer.model) {
        ...
        viewer.addEventListener(av.MODEL_ROOT_LOADED_EVENT, this._onModelLoaded, {
            once: true
        });
    }
...
MeasureExtension.prototype.openCalibrationRequiredDialog = function(initiator) {
    if (this.hasUI) {
        ...
        this.viewer.dispatchEvent({
            type: MeasureCommon.Events.CALIBRATION_REQUIRED_EVENT
        });
    }
    ...
    if (initiator === 'measure') {
        ...
        this.activateInitiator = function() {
            this.viewer.dispatchEvent({
                type: MeasureCommon.Events.FINISHED_CALIBRATION_FOR_DIMENSION_EVENT
            });
            this.activateInitiator = null;
        };
    }
...
MeasureExtension.prototype.enterMeasurementMode = function() {
    ...
    this.viewer.dispatchEvent({
        type: MeasureCommon.Events.MEASUREMENT_MODE_ENTER
    });
    ...
    if (this.hasNotEntered) {
        this.viewer.dispatchEvent({
            type: MeasureCommon.Events.MEASURE_TOOL_ACTIVE
        });
...
MeasureExtension.prototype.exitMeasurementMode = function() {
    ...
    this.viewer.dispatchEvent({
        type: MeasureCommon.Events.MEASUREMENT_MODE_LEAVE
    });
...
MeasureExtension.prototype.onToolbarCreated = function(toolbar) {
    ...
    modelTools.addControl(this.measurementToolbarButton, {
        index: 0
    });
