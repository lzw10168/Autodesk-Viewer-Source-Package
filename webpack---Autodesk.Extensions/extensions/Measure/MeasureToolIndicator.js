...
export function MeasureToolIndicator(viewer, measurement, measureTool) {
    ...
    this.lines = {
        xyz: {
            axis: false,
            material: this._createLineMaterial(DEFAULT_LINE_COLOR),
            className: 'adsk-icon-axis-delta' + '-xyz measure-label-axis-delta' + ' measure-label-axis-xyz'
        },
        x: {
            axis: true,
            material: this._createLineMaterial(X_AXIS_COLOR),
            className: 'adsk-icon-axis-delta' + '-x measure-label-axis-delta' + ' measure-label-axis-x',
            iconText: 'X'
        },
        y: {
            axis: true,
            material: this._createLineMaterial(Y_AXIS_COLOR),
            className: 'adsk-icon-axis-delta' + '-y measure-label-axis-delta' + ' measure-label-axis-y',
            iconText: 'Y'
        },
        z: {
            axis: true,
            material: this._createLineMaterial(Z_AXIS_COLOR),
            className: 'adsk-icon-axis-delta' + '-z measure-label-axis-delta' + ' measure-label-axis-z',
            iconText: 'Z'
        }
    };
...
proto.updateLabelsPosition = function() {
    ...
    if (this.arcLabel) {
        ...
        if (label && this.arcLabel.p1 && this.arcLabel.p2 && isVisible(label)) {
            point = {
                x: (this.arcLabel.p1.x + this.arcLabel.p2.x) / 2,
                y: (this.arcLabel.p1.y + this.arcLabel.p2.y) / 2,
                z: (this.arcLabel.p1.z + this.arcLabel.p2.z) / 2
            };
...
proto.drawXYZLine = function(item) {
    ...
    if (dimensionOffset) {
        ...
        this.segments.push({
            line: this.drawLineSegment(d1, p1, _segmentWidth, item.material),
            p1: d1,
            p2: p1
        });
        this.segments.push({
            line: this.drawLineSegment(p2, d2, _segmentWidth, item.material),
            p1: p2,
            p2: d2
        });
        p1 = d1;
...
proto.onSelectionAreaClicked = function() {
    this.viewer.dispatchEvent({
        type: MeasureCommon.Events.SELECT_MEASUREMENT,
        data: {
