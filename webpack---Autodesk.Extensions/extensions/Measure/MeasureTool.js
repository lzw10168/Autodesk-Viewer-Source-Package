import {
    MeasurementsManager
} from './MeasurementsManager';
import {
    MeasureToolIndicator
} from './MeasureToolIndicator';
import {
    SET_MOUSE_TRACKING
} from './EventTypes';
import {
    EXPLODE_CHANGE_EVENT
} from "../../src/application/EventTypes";
import {
    Matrix4,
    Vector3
} from "three";
...
export function MeasureTool(viewer, options, sharedMeasureConfig, snapper) {
    ...
    this.allPicksSet = function() {
        ...
        _viewer.dispatchEvent({
            ...
            data: {
                type: _currentMeasurement.measurementType,
    ...
    this.activate = function() {
        ...
        _preMeasureState = _viewer.getState({
            objectSet: true
        });
    ...
    this.deleteCurrentMeasurement = function() {
        ...
        if (_deletedMeasurement) {
            _viewer.dispatchEvent({
                ...
                data: {
                    type: _deletedMeasurement.measurementType,
                    id: _deletedMeasurement.id
                }
            });
        }
        ...
        if (_snapper.isSnapped()) {
            ...
            if (event && _viewer.model) {
                ...
                if (x && y) {
                    ...
                    _cursorPosition = canDrawRubberBand ? MeasureCommon.inverseProject({
                        x: x,
                        y: y
                    }, _viewer) : null;
                }
    ...
    this._doConsumeSingleClick = function() {
        ...
        _viewer.dispatchEvent({
            type: SET_MOUSE_TRACKING,
            mode: measurementComplete ? 'end' : 'start'
        });
